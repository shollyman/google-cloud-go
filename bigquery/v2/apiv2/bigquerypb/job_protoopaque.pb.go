// Copyright 2026 Google LLC
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.11
// 	protoc        v6.33.2
// source: google/cloud/bigquery/v2/job.proto

//go:build protoopaque

package bigquerypb

import (
	reflect "reflect"
	unsafe "unsafe"

	_ "google.golang.org/genproto/googleapis/api/annotations"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	emptypb "google.golang.org/protobuf/types/known/emptypb"
	structpb "google.golang.org/protobuf/types/known/structpb"
	wrapperspb "google.golang.org/protobuf/types/known/wrapperspb"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

// Projection is used to control what job information is returned.
type ListJobsRequest_Projection int32

const (
	// Does not include the job configuration
	ListJobsRequest_minimal ListJobsRequest_Projection = 0
	// Does not include the job configuration
	ListJobsRequest_MINIMAL ListJobsRequest_Projection = 0
	// Includes all job data
	ListJobsRequest_full ListJobsRequest_Projection = 1
	// Includes all job data
	ListJobsRequest_FULL ListJobsRequest_Projection = 1
)

// Enum value maps for ListJobsRequest_Projection.
var (
	ListJobsRequest_Projection_name = map[int32]string{
		0: "minimal",
		// Duplicate value: 0: "MINIMAL",
		1: "full",
		// Duplicate value: 1: "FULL",
	}
	ListJobsRequest_Projection_value = map[string]int32{
		"minimal": 0,
		"MINIMAL": 0,
		"full":    1,
		"FULL":    1,
	}
)

func (x ListJobsRequest_Projection) Enum() *ListJobsRequest_Projection {
	p := new(ListJobsRequest_Projection)
	*p = x
	return p
}

func (x ListJobsRequest_Projection) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (ListJobsRequest_Projection) Descriptor() protoreflect.EnumDescriptor {
	return file_google_cloud_bigquery_v2_job_proto_enumTypes[0].Descriptor()
}

func (ListJobsRequest_Projection) Type() protoreflect.EnumType {
	return &file_google_cloud_bigquery_v2_job_proto_enumTypes[0]
}

func (x ListJobsRequest_Projection) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// StateFilter allows filtration by job execution state.
type ListJobsRequest_StateFilter int32

const (
	// Finished jobs
	ListJobsRequest_done ListJobsRequest_StateFilter = 0
	// Finished jobs
	ListJobsRequest_DONE ListJobsRequest_StateFilter = 0
	// Pending jobs
	ListJobsRequest_pending ListJobsRequest_StateFilter = 1
	// Pending jobs
	ListJobsRequest_PENDING ListJobsRequest_StateFilter = 1
	// Running jobs
	ListJobsRequest_running ListJobsRequest_StateFilter = 2
	// Running jobs.
	ListJobsRequest_RUNNING ListJobsRequest_StateFilter = 2
)

// Enum value maps for ListJobsRequest_StateFilter.
var (
	ListJobsRequest_StateFilter_name = map[int32]string{
		0: "done",
		// Duplicate value: 0: "DONE",
		1: "pending",
		// Duplicate value: 1: "PENDING",
		2: "running",
		// Duplicate value: 2: "RUNNING",
	}
	ListJobsRequest_StateFilter_value = map[string]int32{
		"done":    0,
		"DONE":    0,
		"pending": 1,
		"PENDING": 1,
		"running": 2,
		"RUNNING": 2,
	}
)

func (x ListJobsRequest_StateFilter) Enum() *ListJobsRequest_StateFilter {
	p := new(ListJobsRequest_StateFilter)
	*p = x
	return p
}

func (x ListJobsRequest_StateFilter) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (ListJobsRequest_StateFilter) Descriptor() protoreflect.EnumDescriptor {
	return file_google_cloud_bigquery_v2_job_proto_enumTypes[1].Descriptor()
}

func (ListJobsRequest_StateFilter) Type() protoreflect.EnumType {
	return &file_google_cloud_bigquery_v2_job_proto_enumTypes[1]
}

func (x ListJobsRequest_StateFilter) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Job Creation Mode provides different options on job creation.
type QueryRequest_JobCreationMode int32

const (
	// If unspecified JOB_CREATION_REQUIRED is the default.
	QueryRequest_JOB_CREATION_MODE_UNSPECIFIED QueryRequest_JobCreationMode = 0
	// Default. Job creation is always required.
	QueryRequest_JOB_CREATION_REQUIRED QueryRequest_JobCreationMode = 1
	// Job creation is optional. Returning immediate results is prioritized.
	// BigQuery will automatically determine if a Job needs to be created.
	// The conditions under which BigQuery can decide to not create a Job are
	// subject to change. If Job creation is required, JOB_CREATION_REQUIRED
	// mode should be used, which is the default.
	QueryRequest_JOB_CREATION_OPTIONAL QueryRequest_JobCreationMode = 2
)

// Enum value maps for QueryRequest_JobCreationMode.
var (
	QueryRequest_JobCreationMode_name = map[int32]string{
		0: "JOB_CREATION_MODE_UNSPECIFIED",
		1: "JOB_CREATION_REQUIRED",
		2: "JOB_CREATION_OPTIONAL",
	}
	QueryRequest_JobCreationMode_value = map[string]int32{
		"JOB_CREATION_MODE_UNSPECIFIED": 0,
		"JOB_CREATION_REQUIRED":         1,
		"JOB_CREATION_OPTIONAL":         2,
	}
)

func (x QueryRequest_JobCreationMode) Enum() *QueryRequest_JobCreationMode {
	p := new(QueryRequest_JobCreationMode)
	*p = x
	return p
}

func (x QueryRequest_JobCreationMode) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (QueryRequest_JobCreationMode) Descriptor() protoreflect.EnumDescriptor {
	return file_google_cloud_bigquery_v2_job_proto_enumTypes[2].Descriptor()
}

func (QueryRequest_JobCreationMode) Type() protoreflect.EnumType {
	return &file_google_cloud_bigquery_v2_job_proto_enumTypes[2]
}

func (x QueryRequest_JobCreationMode) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// The format of the query results.
//
// This feature is not yet available.
type QueryRequest_QueryResultsFormat int32

const (
	// If unspecified it will default to struct `QueryResponse.rows`
	// (`STRUCT_ENCODING`)
	QueryRequest_QUERY_RESULTS_FORMAT_UNSPECIFIED QueryRequest_QueryResultsFormat = 0
	// Default encoding of results as struct in `QueryResponse.rows`
	QueryRequest_STRUCT_ENCODING QueryRequest_QueryResultsFormat = 1
	// Arrow is a standard open source column-based message format.
	// See https://arrow.apache.org/ for more details.
	QueryRequest_ARROW QueryRequest_QueryResultsFormat = 2
)

// Enum value maps for QueryRequest_QueryResultsFormat.
var (
	QueryRequest_QueryResultsFormat_name = map[int32]string{
		0: "QUERY_RESULTS_FORMAT_UNSPECIFIED",
		1: "STRUCT_ENCODING",
		2: "ARROW",
	}
	QueryRequest_QueryResultsFormat_value = map[string]int32{
		"QUERY_RESULTS_FORMAT_UNSPECIFIED": 0,
		"STRUCT_ENCODING":                  1,
		"ARROW":                            2,
	}
)

func (x QueryRequest_QueryResultsFormat) Enum() *QueryRequest_QueryResultsFormat {
	p := new(QueryRequest_QueryResultsFormat)
	*p = x
	return p
}

func (x QueryRequest_QueryResultsFormat) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (QueryRequest_QueryResultsFormat) Descriptor() protoreflect.EnumDescriptor {
	return file_google_cloud_bigquery_v2_job_proto_enumTypes[3].Descriptor()
}

func (QueryRequest_QueryResultsFormat) Type() protoreflect.EnumType {
	return &file_google_cloud_bigquery_v2_job_proto_enumTypes[3]
}

func (x QueryRequest_QueryResultsFormat) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

type Job struct {
	state                        protoimpl.MessageState `protogen:"opaque.v1"`
	xxx_hidden_Kind              string                 `protobuf:"bytes,1,opt,name=kind,proto3"`
	xxx_hidden_Etag              string                 `protobuf:"bytes,2,opt,name=etag,proto3"`
	xxx_hidden_Id                string                 `protobuf:"bytes,3,opt,name=id,proto3"`
	xxx_hidden_SelfLink          string                 `protobuf:"bytes,4,opt,name=self_link,json=selfLink,proto3"`
	xxx_hidden_UserEmail         string                 `protobuf:"bytes,5,opt,name=user_email,proto3"`
	xxx_hidden_Configuration     *JobConfiguration      `protobuf:"bytes,6,opt,name=configuration,proto3"`
	xxx_hidden_JobReference      *JobReference          `protobuf:"bytes,7,opt,name=job_reference,json=jobReference,proto3"`
	xxx_hidden_Statistics        *JobStatistics         `protobuf:"bytes,8,opt,name=statistics,proto3"`
	xxx_hidden_Status            *JobStatus             `protobuf:"bytes,9,opt,name=status,proto3"`
	xxx_hidden_PrincipalSubject  string                 `protobuf:"bytes,13,opt,name=principal_subject,proto3"`
	xxx_hidden_JobCreationReason *JobCreationReason     `protobuf:"bytes,14,opt,name=job_creation_reason,json=jobCreationReason,proto3"`
	unknownFields                protoimpl.UnknownFields
	sizeCache                    protoimpl.SizeCache
}

func (x *Job) Reset() {
	*x = Job{}
	mi := &file_google_cloud_bigquery_v2_job_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Job) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Job) ProtoMessage() {}

func (x *Job) ProtoReflect() protoreflect.Message {
	mi := &file_google_cloud_bigquery_v2_job_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

func (x *Job) GetKind() string {
	if x != nil {
		return x.xxx_hidden_Kind
	}
	return ""
}

func (x *Job) GetEtag() string {
	if x != nil {
		return x.xxx_hidden_Etag
	}
	return ""
}

func (x *Job) GetId() string {
	if x != nil {
		return x.xxx_hidden_Id
	}
	return ""
}

func (x *Job) GetSelfLink() string {
	if x != nil {
		return x.xxx_hidden_SelfLink
	}
	return ""
}

func (x *Job) GetUserEmail() string {
	if x != nil {
		return x.xxx_hidden_UserEmail
	}
	return ""
}

func (x *Job) GetConfiguration() *JobConfiguration {
	if x != nil {
		return x.xxx_hidden_Configuration
	}
	return nil
}

func (x *Job) GetJobReference() *JobReference {
	if x != nil {
		return x.xxx_hidden_JobReference
	}
	return nil
}

func (x *Job) GetStatistics() *JobStatistics {
	if x != nil {
		return x.xxx_hidden_Statistics
	}
	return nil
}

func (x *Job) GetStatus() *JobStatus {
	if x != nil {
		return x.xxx_hidden_Status
	}
	return nil
}

func (x *Job) GetPrincipalSubject() string {
	if x != nil {
		return x.xxx_hidden_PrincipalSubject
	}
	return ""
}

func (x *Job) GetJobCreationReason() *JobCreationReason {
	if x != nil {
		return x.xxx_hidden_JobCreationReason
	}
	return nil
}

func (x *Job) SetKind(v string) {
	x.xxx_hidden_Kind = v
}

func (x *Job) SetEtag(v string) {
	x.xxx_hidden_Etag = v
}

func (x *Job) SetId(v string) {
	x.xxx_hidden_Id = v
}

func (x *Job) SetSelfLink(v string) {
	x.xxx_hidden_SelfLink = v
}

func (x *Job) SetUserEmail(v string) {
	x.xxx_hidden_UserEmail = v
}

func (x *Job) SetConfiguration(v *JobConfiguration) {
	x.xxx_hidden_Configuration = v
}

func (x *Job) SetJobReference(v *JobReference) {
	x.xxx_hidden_JobReference = v
}

func (x *Job) SetStatistics(v *JobStatistics) {
	x.xxx_hidden_Statistics = v
}

func (x *Job) SetStatus(v *JobStatus) {
	x.xxx_hidden_Status = v
}

func (x *Job) SetPrincipalSubject(v string) {
	x.xxx_hidden_PrincipalSubject = v
}

func (x *Job) SetJobCreationReason(v *JobCreationReason) {
	x.xxx_hidden_JobCreationReason = v
}

func (x *Job) HasConfiguration() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_Configuration != nil
}

func (x *Job) HasJobReference() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_JobReference != nil
}

func (x *Job) HasStatistics() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_Statistics != nil
}

func (x *Job) HasStatus() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_Status != nil
}

func (x *Job) HasJobCreationReason() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_JobCreationReason != nil
}

func (x *Job) ClearConfiguration() {
	x.xxx_hidden_Configuration = nil
}

func (x *Job) ClearJobReference() {
	x.xxx_hidden_JobReference = nil
}

func (x *Job) ClearStatistics() {
	x.xxx_hidden_Statistics = nil
}

func (x *Job) ClearStatus() {
	x.xxx_hidden_Status = nil
}

func (x *Job) ClearJobCreationReason() {
	x.xxx_hidden_JobCreationReason = nil
}

type Job_builder struct {
	_ [0]func() // Prevents comparability and use of unkeyed literals for the builder.

	// Output only. The type of the resource.
	Kind string
	// Output only. A hash of this resource.
	Etag string
	// Output only. Opaque ID field of the job.
	Id string
	// Output only. A URL that can be used to access the resource again.
	SelfLink string
	// Output only. Email address of the user who ran the job.
	UserEmail string
	// Required. Describes the job configuration.
	Configuration *JobConfiguration
	// Optional. Reference describing the unique-per-user name of the job.
	JobReference *JobReference
	// Output only. Information about the job, including starting time and ending
	// time of the job.
	Statistics *JobStatistics
	// Output only. The status of this job. Examine this value when polling an
	// asynchronous job to see if the job is complete.
	Status *JobStatus
	// Output only. [Full-projection-only] String representation of identity of
	// requesting party. Populated for both first- and third-party identities.
	// Only present for APIs that support third-party identities.
	PrincipalSubject string
	// Output only. The reason why a Job was created.
	JobCreationReason *JobCreationReason
}

func (b0 Job_builder) Build() *Job {
	m0 := &Job{}
	b, x := &b0, m0
	_, _ = b, x
	x.xxx_hidden_Kind = b.Kind
	x.xxx_hidden_Etag = b.Etag
	x.xxx_hidden_Id = b.Id
	x.xxx_hidden_SelfLink = b.SelfLink
	x.xxx_hidden_UserEmail = b.UserEmail
	x.xxx_hidden_Configuration = b.Configuration
	x.xxx_hidden_JobReference = b.JobReference
	x.xxx_hidden_Statistics = b.Statistics
	x.xxx_hidden_Status = b.Status
	x.xxx_hidden_PrincipalSubject = b.PrincipalSubject
	x.xxx_hidden_JobCreationReason = b.JobCreationReason
	return m0
}

// Describes format of a jobs cancellation request.
type CancelJobRequest struct {
	state                protoimpl.MessageState `protogen:"opaque.v1"`
	xxx_hidden_ProjectId string                 `protobuf:"bytes,1,opt,name=project_id,json=projectId,proto3"`
	xxx_hidden_JobId     string                 `protobuf:"bytes,2,opt,name=job_id,json=jobId,proto3"`
	xxx_hidden_Location  string                 `protobuf:"bytes,3,opt,name=location,proto3"`
	unknownFields        protoimpl.UnknownFields
	sizeCache            protoimpl.SizeCache
}

func (x *CancelJobRequest) Reset() {
	*x = CancelJobRequest{}
	mi := &file_google_cloud_bigquery_v2_job_proto_msgTypes[1]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CancelJobRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CancelJobRequest) ProtoMessage() {}

func (x *CancelJobRequest) ProtoReflect() protoreflect.Message {
	mi := &file_google_cloud_bigquery_v2_job_proto_msgTypes[1]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

func (x *CancelJobRequest) GetProjectId() string {
	if x != nil {
		return x.xxx_hidden_ProjectId
	}
	return ""
}

func (x *CancelJobRequest) GetJobId() string {
	if x != nil {
		return x.xxx_hidden_JobId
	}
	return ""
}

func (x *CancelJobRequest) GetLocation() string {
	if x != nil {
		return x.xxx_hidden_Location
	}
	return ""
}

func (x *CancelJobRequest) SetProjectId(v string) {
	x.xxx_hidden_ProjectId = v
}

func (x *CancelJobRequest) SetJobId(v string) {
	x.xxx_hidden_JobId = v
}

func (x *CancelJobRequest) SetLocation(v string) {
	x.xxx_hidden_Location = v
}

type CancelJobRequest_builder struct {
	_ [0]func() // Prevents comparability and use of unkeyed literals for the builder.

	// Required. Project ID of the job to cancel
	ProjectId string
	// Required. Job ID of the job to cancel
	JobId string
	// The geographic location of the job. You must
	// [specify the
	// location](https://cloud.google.com/bigquery/docs/locations#specify_locations)
	// to run the job for the following scenarios:
	//
	//   - If the location to run a job is not in the `us` or
	//     the `eu` multi-regional location
	//   - If the job's location is in a single region (for example,
	//     `us-central1`)
	Location string
}

func (b0 CancelJobRequest_builder) Build() *CancelJobRequest {
	m0 := &CancelJobRequest{}
	b, x := &b0, m0
	_, _ = b, x
	x.xxx_hidden_ProjectId = b.ProjectId
	x.xxx_hidden_JobId = b.JobId
	x.xxx_hidden_Location = b.Location
	return m0
}

// Describes format of a jobs cancellation response.
type JobCancelResponse struct {
	state           protoimpl.MessageState `protogen:"opaque.v1"`
	xxx_hidden_Kind string                 `protobuf:"bytes,1,opt,name=kind,proto3"`
	xxx_hidden_Job  *Job                   `protobuf:"bytes,2,opt,name=job,proto3"`
	unknownFields   protoimpl.UnknownFields
	sizeCache       protoimpl.SizeCache
}

func (x *JobCancelResponse) Reset() {
	*x = JobCancelResponse{}
	mi := &file_google_cloud_bigquery_v2_job_proto_msgTypes[2]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *JobCancelResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*JobCancelResponse) ProtoMessage() {}

func (x *JobCancelResponse) ProtoReflect() protoreflect.Message {
	mi := &file_google_cloud_bigquery_v2_job_proto_msgTypes[2]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

func (x *JobCancelResponse) GetKind() string {
	if x != nil {
		return x.xxx_hidden_Kind
	}
	return ""
}

func (x *JobCancelResponse) GetJob() *Job {
	if x != nil {
		return x.xxx_hidden_Job
	}
	return nil
}

func (x *JobCancelResponse) SetKind(v string) {
	x.xxx_hidden_Kind = v
}

func (x *JobCancelResponse) SetJob(v *Job) {
	x.xxx_hidden_Job = v
}

func (x *JobCancelResponse) HasJob() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_Job != nil
}

func (x *JobCancelResponse) ClearJob() {
	x.xxx_hidden_Job = nil
}

type JobCancelResponse_builder struct {
	_ [0]func() // Prevents comparability and use of unkeyed literals for the builder.

	// The resource type of the response.
	Kind string
	// The final state of the job.
	Job *Job
}

func (b0 JobCancelResponse_builder) Build() *JobCancelResponse {
	m0 := &JobCancelResponse{}
	b, x := &b0, m0
	_, _ = b, x
	x.xxx_hidden_Kind = b.Kind
	x.xxx_hidden_Job = b.Job
	return m0
}

// Describes format of a jobs get request.
type GetJobRequest struct {
	state                protoimpl.MessageState `protogen:"opaque.v1"`
	xxx_hidden_ProjectId string                 `protobuf:"bytes,1,opt,name=project_id,json=projectId,proto3"`
	xxx_hidden_JobId     string                 `protobuf:"bytes,2,opt,name=job_id,json=jobId,proto3"`
	xxx_hidden_Location  string                 `protobuf:"bytes,3,opt,name=location,proto3"`
	unknownFields        protoimpl.UnknownFields
	sizeCache            protoimpl.SizeCache
}

func (x *GetJobRequest) Reset() {
	*x = GetJobRequest{}
	mi := &file_google_cloud_bigquery_v2_job_proto_msgTypes[3]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetJobRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetJobRequest) ProtoMessage() {}

func (x *GetJobRequest) ProtoReflect() protoreflect.Message {
	mi := &file_google_cloud_bigquery_v2_job_proto_msgTypes[3]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

func (x *GetJobRequest) GetProjectId() string {
	if x != nil {
		return x.xxx_hidden_ProjectId
	}
	return ""
}

func (x *GetJobRequest) GetJobId() string {
	if x != nil {
		return x.xxx_hidden_JobId
	}
	return ""
}

func (x *GetJobRequest) GetLocation() string {
	if x != nil {
		return x.xxx_hidden_Location
	}
	return ""
}

func (x *GetJobRequest) SetProjectId(v string) {
	x.xxx_hidden_ProjectId = v
}

func (x *GetJobRequest) SetJobId(v string) {
	x.xxx_hidden_JobId = v
}

func (x *GetJobRequest) SetLocation(v string) {
	x.xxx_hidden_Location = v
}

type GetJobRequest_builder struct {
	_ [0]func() // Prevents comparability and use of unkeyed literals for the builder.

	// Required. Project ID of the requested job.
	ProjectId string
	// Required. Job ID of the requested job.
	JobId string
	// The geographic location of the job. You must specify the location to run
	// the job for the following scenarios:
	//
	//   - If the location to run a job is not in the `us` or
	//     the `eu` multi-regional location
	//   - If the job's location is in a single region (for example,
	//     `us-central1`)
	//
	// For more information, see how to
	// [specify
	// locations](https://cloud.google.com/bigquery/docs/locations#specify_locations).
	Location string
}

func (b0 GetJobRequest_builder) Build() *GetJobRequest {
	m0 := &GetJobRequest{}
	b, x := &b0, m0
	_, _ = b, x
	x.xxx_hidden_ProjectId = b.ProjectId
	x.xxx_hidden_JobId = b.JobId
	x.xxx_hidden_Location = b.Location
	return m0
}

// Describes format of a job insertion request.
type InsertJobRequest struct {
	state                protoimpl.MessageState `protogen:"opaque.v1"`
	xxx_hidden_ProjectId string                 `protobuf:"bytes,1,opt,name=project_id,json=projectId,proto3"`
	xxx_hidden_Job       *Job                   `protobuf:"bytes,3,opt,name=job,proto3"`
	unknownFields        protoimpl.UnknownFields
	sizeCache            protoimpl.SizeCache
}

func (x *InsertJobRequest) Reset() {
	*x = InsertJobRequest{}
	mi := &file_google_cloud_bigquery_v2_job_proto_msgTypes[4]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *InsertJobRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*InsertJobRequest) ProtoMessage() {}

func (x *InsertJobRequest) ProtoReflect() protoreflect.Message {
	mi := &file_google_cloud_bigquery_v2_job_proto_msgTypes[4]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

func (x *InsertJobRequest) GetProjectId() string {
	if x != nil {
		return x.xxx_hidden_ProjectId
	}
	return ""
}

func (x *InsertJobRequest) GetJob() *Job {
	if x != nil {
		return x.xxx_hidden_Job
	}
	return nil
}

func (x *InsertJobRequest) SetProjectId(v string) {
	x.xxx_hidden_ProjectId = v
}

func (x *InsertJobRequest) SetJob(v *Job) {
	x.xxx_hidden_Job = v
}

func (x *InsertJobRequest) HasJob() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_Job != nil
}

func (x *InsertJobRequest) ClearJob() {
	x.xxx_hidden_Job = nil
}

type InsertJobRequest_builder struct {
	_ [0]func() // Prevents comparability and use of unkeyed literals for the builder.

	// Project ID of project that will be billed for the job.
	ProjectId string
	// Jobs resource to insert.
	Job *Job
}

func (b0 InsertJobRequest_builder) Build() *InsertJobRequest {
	m0 := &InsertJobRequest{}
	b, x := &b0, m0
	_, _ = b, x
	x.xxx_hidden_ProjectId = b.ProjectId
	x.xxx_hidden_Job = b.Job
	return m0
}

// Describes the format of a jobs deletion request.
type DeleteJobRequest struct {
	state                protoimpl.MessageState `protogen:"opaque.v1"`
	xxx_hidden_ProjectId string                 `protobuf:"bytes,1,opt,name=project_id,json=projectId,proto3"`
	xxx_hidden_JobId     string                 `protobuf:"bytes,2,opt,name=job_id,json=jobId,proto3"`
	xxx_hidden_Location  string                 `protobuf:"bytes,3,opt,name=location,proto3"`
	unknownFields        protoimpl.UnknownFields
	sizeCache            protoimpl.SizeCache
}

func (x *DeleteJobRequest) Reset() {
	*x = DeleteJobRequest{}
	mi := &file_google_cloud_bigquery_v2_job_proto_msgTypes[5]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DeleteJobRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DeleteJobRequest) ProtoMessage() {}

func (x *DeleteJobRequest) ProtoReflect() protoreflect.Message {
	mi := &file_google_cloud_bigquery_v2_job_proto_msgTypes[5]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

func (x *DeleteJobRequest) GetProjectId() string {
	if x != nil {
		return x.xxx_hidden_ProjectId
	}
	return ""
}

func (x *DeleteJobRequest) GetJobId() string {
	if x != nil {
		return x.xxx_hidden_JobId
	}
	return ""
}

func (x *DeleteJobRequest) GetLocation() string {
	if x != nil {
		return x.xxx_hidden_Location
	}
	return ""
}

func (x *DeleteJobRequest) SetProjectId(v string) {
	x.xxx_hidden_ProjectId = v
}

func (x *DeleteJobRequest) SetJobId(v string) {
	x.xxx_hidden_JobId = v
}

func (x *DeleteJobRequest) SetLocation(v string) {
	x.xxx_hidden_Location = v
}

type DeleteJobRequest_builder struct {
	_ [0]func() // Prevents comparability and use of unkeyed literals for the builder.

	// Required. Project ID of the job for which metadata is to be deleted.
	ProjectId string
	// Required. Job ID of the job for which metadata is to be deleted. If this is
	// a parent job which has child jobs, the metadata from all child jobs will be
	// deleted as well. Direct deletion of the metadata of child jobs is not
	// allowed.
	JobId string
	// The geographic location of the job. Required.
	//
	// For more information, see how to
	// [specify
	// locations](https://cloud.google.com/bigquery/docs/locations#specify_locations).
	Location string
}

func (b0 DeleteJobRequest_builder) Build() *DeleteJobRequest {
	m0 := &DeleteJobRequest{}
	b, x := &b0, m0
	_, _ = b, x
	x.xxx_hidden_ProjectId = b.ProjectId
	x.xxx_hidden_JobId = b.JobId
	x.xxx_hidden_Location = b.Location
	return m0
}

// Describes the format of the list jobs request.
type ListJobsRequest struct {
	state                      protoimpl.MessageState        `protogen:"opaque.v1"`
	xxx_hidden_ProjectId       string                        `protobuf:"bytes,1,opt,name=project_id,json=projectId,proto3"`
	xxx_hidden_AllUsers        bool                          `protobuf:"varint,2,opt,name=all_users,json=allUsers,proto3"`
	xxx_hidden_MaxResults      *wrapperspb.Int32Value        `protobuf:"bytes,3,opt,name=max_results,json=maxResults,proto3"`
	xxx_hidden_MinCreationTime uint64                        `protobuf:"varint,4,opt,name=min_creation_time,json=minCreationTime,proto3"`
	xxx_hidden_MaxCreationTime *wrapperspb.UInt64Value       `protobuf:"bytes,5,opt,name=max_creation_time,json=maxCreationTime,proto3"`
	xxx_hidden_PageToken       string                        `protobuf:"bytes,6,opt,name=page_token,json=pageToken,proto3"`
	xxx_hidden_Projection      ListJobsRequest_Projection    `protobuf:"varint,7,opt,name=projection,proto3,enum=google.cloud.bigquery.v2.ListJobsRequest_Projection"`
	xxx_hidden_StateFilter     []ListJobsRequest_StateFilter `protobuf:"varint,8,rep,packed,name=state_filter,json=stateFilter,proto3,enum=google.cloud.bigquery.v2.ListJobsRequest_StateFilter"`
	xxx_hidden_ParentJobId     string                        `protobuf:"bytes,9,opt,name=parent_job_id,json=parentJobId,proto3"`
	unknownFields              protoimpl.UnknownFields
	sizeCache                  protoimpl.SizeCache
}

func (x *ListJobsRequest) Reset() {
	*x = ListJobsRequest{}
	mi := &file_google_cloud_bigquery_v2_job_proto_msgTypes[6]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListJobsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListJobsRequest) ProtoMessage() {}

func (x *ListJobsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_google_cloud_bigquery_v2_job_proto_msgTypes[6]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

func (x *ListJobsRequest) GetProjectId() string {
	if x != nil {
		return x.xxx_hidden_ProjectId
	}
	return ""
}

func (x *ListJobsRequest) GetAllUsers() bool {
	if x != nil {
		return x.xxx_hidden_AllUsers
	}
	return false
}

func (x *ListJobsRequest) GetMaxResults() *wrapperspb.Int32Value {
	if x != nil {
		return x.xxx_hidden_MaxResults
	}
	return nil
}

func (x *ListJobsRequest) GetMinCreationTime() uint64 {
	if x != nil {
		return x.xxx_hidden_MinCreationTime
	}
	return 0
}

func (x *ListJobsRequest) GetMaxCreationTime() *wrapperspb.UInt64Value {
	if x != nil {
		return x.xxx_hidden_MaxCreationTime
	}
	return nil
}

func (x *ListJobsRequest) GetPageToken() string {
	if x != nil {
		return x.xxx_hidden_PageToken
	}
	return ""
}

func (x *ListJobsRequest) GetProjection() ListJobsRequest_Projection {
	if x != nil {
		return x.xxx_hidden_Projection
	}
	return ListJobsRequest_minimal
}

func (x *ListJobsRequest) GetStateFilter() []ListJobsRequest_StateFilter {
	if x != nil {
		return x.xxx_hidden_StateFilter
	}
	return nil
}

func (x *ListJobsRequest) GetParentJobId() string {
	if x != nil {
		return x.xxx_hidden_ParentJobId
	}
	return ""
}

func (x *ListJobsRequest) SetProjectId(v string) {
	x.xxx_hidden_ProjectId = v
}

func (x *ListJobsRequest) SetAllUsers(v bool) {
	x.xxx_hidden_AllUsers = v
}

func (x *ListJobsRequest) SetMaxResults(v *wrapperspb.Int32Value) {
	x.xxx_hidden_MaxResults = v
}

func (x *ListJobsRequest) SetMinCreationTime(v uint64) {
	x.xxx_hidden_MinCreationTime = v
}

func (x *ListJobsRequest) SetMaxCreationTime(v *wrapperspb.UInt64Value) {
	x.xxx_hidden_MaxCreationTime = v
}

func (x *ListJobsRequest) SetPageToken(v string) {
	x.xxx_hidden_PageToken = v
}

func (x *ListJobsRequest) SetProjection(v ListJobsRequest_Projection) {
	x.xxx_hidden_Projection = v
}

func (x *ListJobsRequest) SetStateFilter(v []ListJobsRequest_StateFilter) {
	x.xxx_hidden_StateFilter = v
}

func (x *ListJobsRequest) SetParentJobId(v string) {
	x.xxx_hidden_ParentJobId = v
}

func (x *ListJobsRequest) HasMaxResults() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_MaxResults != nil
}

func (x *ListJobsRequest) HasMaxCreationTime() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_MaxCreationTime != nil
}

func (x *ListJobsRequest) ClearMaxResults() {
	x.xxx_hidden_MaxResults = nil
}

func (x *ListJobsRequest) ClearMaxCreationTime() {
	x.xxx_hidden_MaxCreationTime = nil
}

type ListJobsRequest_builder struct {
	_ [0]func() // Prevents comparability and use of unkeyed literals for the builder.

	// Project ID of the jobs to list.
	ProjectId string
	// Whether to display jobs owned by all users in the project. Default False.
	AllUsers bool
	// The maximum number of results to return in a single response page.
	// Leverage the page tokens to iterate through the entire collection.
	MaxResults *wrapperspb.Int32Value
	// Min value for job creation time, in milliseconds since the POSIX epoch.
	// If set, only jobs created after or at this timestamp are returned.
	MinCreationTime uint64
	// Max value for job creation time, in milliseconds since the POSIX epoch.
	// If set, only jobs created before or at this timestamp are returned.
	MaxCreationTime *wrapperspb.UInt64Value
	// Page token, returned by a previous call, to request the next page of
	// results.
	PageToken string
	// Restrict information returned to a set of selected fields
	Projection ListJobsRequest_Projection
	// Filter for job state
	StateFilter []ListJobsRequest_StateFilter
	// If set, show only child jobs of the specified parent.  Otherwise, show all
	// top-level jobs.
	ParentJobId string
}

func (b0 ListJobsRequest_builder) Build() *ListJobsRequest {
	m0 := &ListJobsRequest{}
	b, x := &b0, m0
	_, _ = b, x
	x.xxx_hidden_ProjectId = b.ProjectId
	x.xxx_hidden_AllUsers = b.AllUsers
	x.xxx_hidden_MaxResults = b.MaxResults
	x.xxx_hidden_MinCreationTime = b.MinCreationTime
	x.xxx_hidden_MaxCreationTime = b.MaxCreationTime
	x.xxx_hidden_PageToken = b.PageToken
	x.xxx_hidden_Projection = b.Projection
	x.xxx_hidden_StateFilter = b.StateFilter
	x.xxx_hidden_ParentJobId = b.ParentJobId
	return m0
}

// ListFormatJob is a partial projection of job information returned as part
// of a jobs.list response.
type ListFormatJob struct {
	state                       protoimpl.MessageState `protogen:"opaque.v1"`
	xxx_hidden_Id               string                 `protobuf:"bytes,1,opt,name=id,proto3"`
	xxx_hidden_Kind             string                 `protobuf:"bytes,2,opt,name=kind,proto3"`
	xxx_hidden_JobReference     *JobReference          `protobuf:"bytes,3,opt,name=job_reference,json=jobReference,proto3"`
	xxx_hidden_State            string                 `protobuf:"bytes,4,opt,name=state,proto3"`
	xxx_hidden_ErrorResult      *ErrorProto            `protobuf:"bytes,5,opt,name=error_result,json=errorResult,proto3"`
	xxx_hidden_Statistics       *JobStatistics         `protobuf:"bytes,6,opt,name=statistics,proto3"`
	xxx_hidden_Configuration    *JobConfiguration      `protobuf:"bytes,7,opt,name=configuration,proto3"`
	xxx_hidden_Status           *JobStatus             `protobuf:"bytes,8,opt,name=status,proto3"`
	xxx_hidden_UserEmail        string                 `protobuf:"bytes,9,opt,name=user_email,proto3"`
	xxx_hidden_PrincipalSubject string                 `protobuf:"bytes,10,opt,name=principal_subject,proto3"`
	unknownFields               protoimpl.UnknownFields
	sizeCache                   protoimpl.SizeCache
}

func (x *ListFormatJob) Reset() {
	*x = ListFormatJob{}
	mi := &file_google_cloud_bigquery_v2_job_proto_msgTypes[7]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListFormatJob) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListFormatJob) ProtoMessage() {}

func (x *ListFormatJob) ProtoReflect() protoreflect.Message {
	mi := &file_google_cloud_bigquery_v2_job_proto_msgTypes[7]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

func (x *ListFormatJob) GetId() string {
	if x != nil {
		return x.xxx_hidden_Id
	}
	return ""
}

func (x *ListFormatJob) GetKind() string {
	if x != nil {
		return x.xxx_hidden_Kind
	}
	return ""
}

func (x *ListFormatJob) GetJobReference() *JobReference {
	if x != nil {
		return x.xxx_hidden_JobReference
	}
	return nil
}

func (x *ListFormatJob) GetState() string {
	if x != nil {
		return x.xxx_hidden_State
	}
	return ""
}

func (x *ListFormatJob) GetErrorResult() *ErrorProto {
	if x != nil {
		return x.xxx_hidden_ErrorResult
	}
	return nil
}

func (x *ListFormatJob) GetStatistics() *JobStatistics {
	if x != nil {
		return x.xxx_hidden_Statistics
	}
	return nil
}

func (x *ListFormatJob) GetConfiguration() *JobConfiguration {
	if x != nil {
		return x.xxx_hidden_Configuration
	}
	return nil
}

func (x *ListFormatJob) GetStatus() *JobStatus {
	if x != nil {
		return x.xxx_hidden_Status
	}
	return nil
}

func (x *ListFormatJob) GetUserEmail() string {
	if x != nil {
		return x.xxx_hidden_UserEmail
	}
	return ""
}

func (x *ListFormatJob) GetPrincipalSubject() string {
	if x != nil {
		return x.xxx_hidden_PrincipalSubject
	}
	return ""
}

func (x *ListFormatJob) SetId(v string) {
	x.xxx_hidden_Id = v
}

func (x *ListFormatJob) SetKind(v string) {
	x.xxx_hidden_Kind = v
}

func (x *ListFormatJob) SetJobReference(v *JobReference) {
	x.xxx_hidden_JobReference = v
}

func (x *ListFormatJob) SetState(v string) {
	x.xxx_hidden_State = v
}

func (x *ListFormatJob) SetErrorResult(v *ErrorProto) {
	x.xxx_hidden_ErrorResult = v
}

func (x *ListFormatJob) SetStatistics(v *JobStatistics) {
	x.xxx_hidden_Statistics = v
}

func (x *ListFormatJob) SetConfiguration(v *JobConfiguration) {
	x.xxx_hidden_Configuration = v
}

func (x *ListFormatJob) SetStatus(v *JobStatus) {
	x.xxx_hidden_Status = v
}

func (x *ListFormatJob) SetUserEmail(v string) {
	x.xxx_hidden_UserEmail = v
}

func (x *ListFormatJob) SetPrincipalSubject(v string) {
	x.xxx_hidden_PrincipalSubject = v
}

func (x *ListFormatJob) HasJobReference() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_JobReference != nil
}

func (x *ListFormatJob) HasErrorResult() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_ErrorResult != nil
}

func (x *ListFormatJob) HasStatistics() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_Statistics != nil
}

func (x *ListFormatJob) HasConfiguration() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_Configuration != nil
}

func (x *ListFormatJob) HasStatus() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_Status != nil
}

func (x *ListFormatJob) ClearJobReference() {
	x.xxx_hidden_JobReference = nil
}

func (x *ListFormatJob) ClearErrorResult() {
	x.xxx_hidden_ErrorResult = nil
}

func (x *ListFormatJob) ClearStatistics() {
	x.xxx_hidden_Statistics = nil
}

func (x *ListFormatJob) ClearConfiguration() {
	x.xxx_hidden_Configuration = nil
}

func (x *ListFormatJob) ClearStatus() {
	x.xxx_hidden_Status = nil
}

type ListFormatJob_builder struct {
	_ [0]func() // Prevents comparability and use of unkeyed literals for the builder.

	// Unique opaque ID of the job.
	Id string
	// The resource type.
	Kind string
	// Unique opaque ID of the job.
	JobReference *JobReference
	// Running state of the job. When the state is DONE, errorResult can be
	// checked to determine whether the job succeeded or failed.
	State string
	// A result object that will be present only if the job has failed.
	ErrorResult *ErrorProto
	// Output only. Information about the job, including starting time and ending
	// time of the job.
	Statistics *JobStatistics
	// Required. Describes the job configuration.
	Configuration *JobConfiguration
	// [Full-projection-only] Describes the status of this job.
	Status *JobStatus
	// [Full-projection-only] Email address of the user who ran the job.
	UserEmail string
	// [Full-projection-only] String representation of identity of requesting
	// party. Populated for both first- and third-party identities. Only present
	// for APIs that support third-party identities.
	PrincipalSubject string
}

func (b0 ListFormatJob_builder) Build() *ListFormatJob {
	m0 := &ListFormatJob{}
	b, x := &b0, m0
	_, _ = b, x
	x.xxx_hidden_Id = b.Id
	x.xxx_hidden_Kind = b.Kind
	x.xxx_hidden_JobReference = b.JobReference
	x.xxx_hidden_State = b.State
	x.xxx_hidden_ErrorResult = b.ErrorResult
	x.xxx_hidden_Statistics = b.Statistics
	x.xxx_hidden_Configuration = b.Configuration
	x.xxx_hidden_Status = b.Status
	x.xxx_hidden_UserEmail = b.UserEmail
	x.xxx_hidden_PrincipalSubject = b.PrincipalSubject
	return m0
}

// JobList is the response format for a jobs.list call.
type JobList struct {
	state                    protoimpl.MessageState `protogen:"opaque.v1"`
	xxx_hidden_Etag          string                 `protobuf:"bytes,1,opt,name=etag,proto3"`
	xxx_hidden_Kind          string                 `protobuf:"bytes,2,opt,name=kind,proto3"`
	xxx_hidden_NextPageToken string                 `protobuf:"bytes,3,opt,name=next_page_token,json=nextPageToken,proto3"`
	xxx_hidden_Jobs          *[]*ListFormatJob      `protobuf:"bytes,4,rep,name=jobs,proto3"`
	xxx_hidden_Unreachable   []string               `protobuf:"bytes,5,rep,name=unreachable,proto3"`
	unknownFields            protoimpl.UnknownFields
	sizeCache                protoimpl.SizeCache
}

func (x *JobList) Reset() {
	*x = JobList{}
	mi := &file_google_cloud_bigquery_v2_job_proto_msgTypes[8]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *JobList) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*JobList) ProtoMessage() {}

func (x *JobList) ProtoReflect() protoreflect.Message {
	mi := &file_google_cloud_bigquery_v2_job_proto_msgTypes[8]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

func (x *JobList) GetEtag() string {
	if x != nil {
		return x.xxx_hidden_Etag
	}
	return ""
}

func (x *JobList) GetKind() string {
	if x != nil {
		return x.xxx_hidden_Kind
	}
	return ""
}

func (x *JobList) GetNextPageToken() string {
	if x != nil {
		return x.xxx_hidden_NextPageToken
	}
	return ""
}

func (x *JobList) GetJobs() []*ListFormatJob {
	if x != nil {
		if x.xxx_hidden_Jobs != nil {
			return *x.xxx_hidden_Jobs
		}
	}
	return nil
}

func (x *JobList) GetUnreachable() []string {
	if x != nil {
		return x.xxx_hidden_Unreachable
	}
	return nil
}

func (x *JobList) SetEtag(v string) {
	x.xxx_hidden_Etag = v
}

func (x *JobList) SetKind(v string) {
	x.xxx_hidden_Kind = v
}

func (x *JobList) SetNextPageToken(v string) {
	x.xxx_hidden_NextPageToken = v
}

func (x *JobList) SetJobs(v []*ListFormatJob) {
	x.xxx_hidden_Jobs = &v
}

func (x *JobList) SetUnreachable(v []string) {
	x.xxx_hidden_Unreachable = v
}

type JobList_builder struct {
	_ [0]func() // Prevents comparability and use of unkeyed literals for the builder.

	// A hash of this page of results.
	Etag string
	// The resource type of the response.
	Kind string
	// A token to request the next page of results.
	NextPageToken string
	// List of jobs that were requested.
	Jobs []*ListFormatJob
	// A list of skipped locations that were unreachable. For more information
	// about BigQuery locations, see:
	// https://cloud.google.com/bigquery/docs/locations. Example: "europe-west5"
	Unreachable []string
}

func (b0 JobList_builder) Build() *JobList {
	m0 := &JobList{}
	b, x := &b0, m0
	_, _ = b, x
	x.xxx_hidden_Etag = b.Etag
	x.xxx_hidden_Kind = b.Kind
	x.xxx_hidden_NextPageToken = b.NextPageToken
	x.xxx_hidden_Jobs = &b.Jobs
	x.xxx_hidden_Unreachable = b.Unreachable
	return m0
}

// Request object of GetQueryResults.
type GetQueryResultsRequest struct {
	state                    protoimpl.MessageState  `protogen:"opaque.v1"`
	xxx_hidden_ProjectId     string                  `protobuf:"bytes,1,opt,name=project_id,json=projectId,proto3"`
	xxx_hidden_JobId         string                  `protobuf:"bytes,2,opt,name=job_id,json=jobId,proto3"`
	xxx_hidden_StartIndex    *wrapperspb.UInt64Value `protobuf:"bytes,3,opt,name=start_index,json=startIndex,proto3"`
	xxx_hidden_PageToken     string                  `protobuf:"bytes,4,opt,name=page_token,json=pageToken,proto3"`
	xxx_hidden_MaxResults    *wrapperspb.UInt32Value `protobuf:"bytes,5,opt,name=max_results,json=maxResults,proto3"`
	xxx_hidden_TimeoutMs     *wrapperspb.UInt32Value `protobuf:"bytes,6,opt,name=timeout_ms,json=timeoutMs,proto3"`
	xxx_hidden_Location      string                  `protobuf:"bytes,7,opt,name=location,proto3"`
	xxx_hidden_FormatOptions *DataFormatOptions      `protobuf:"bytes,8,opt,name=format_options,json=formatOptions,proto3"`
	unknownFields            protoimpl.UnknownFields
	sizeCache                protoimpl.SizeCache
}

func (x *GetQueryResultsRequest) Reset() {
	*x = GetQueryResultsRequest{}
	mi := &file_google_cloud_bigquery_v2_job_proto_msgTypes[9]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetQueryResultsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetQueryResultsRequest) ProtoMessage() {}

func (x *GetQueryResultsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_google_cloud_bigquery_v2_job_proto_msgTypes[9]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

func (x *GetQueryResultsRequest) GetProjectId() string {
	if x != nil {
		return x.xxx_hidden_ProjectId
	}
	return ""
}

func (x *GetQueryResultsRequest) GetJobId() string {
	if x != nil {
		return x.xxx_hidden_JobId
	}
	return ""
}

func (x *GetQueryResultsRequest) GetStartIndex() *wrapperspb.UInt64Value {
	if x != nil {
		return x.xxx_hidden_StartIndex
	}
	return nil
}

func (x *GetQueryResultsRequest) GetPageToken() string {
	if x != nil {
		return x.xxx_hidden_PageToken
	}
	return ""
}

func (x *GetQueryResultsRequest) GetMaxResults() *wrapperspb.UInt32Value {
	if x != nil {
		return x.xxx_hidden_MaxResults
	}
	return nil
}

func (x *GetQueryResultsRequest) GetTimeoutMs() *wrapperspb.UInt32Value {
	if x != nil {
		return x.xxx_hidden_TimeoutMs
	}
	return nil
}

func (x *GetQueryResultsRequest) GetLocation() string {
	if x != nil {
		return x.xxx_hidden_Location
	}
	return ""
}

func (x *GetQueryResultsRequest) GetFormatOptions() *DataFormatOptions {
	if x != nil {
		return x.xxx_hidden_FormatOptions
	}
	return nil
}

func (x *GetQueryResultsRequest) SetProjectId(v string) {
	x.xxx_hidden_ProjectId = v
}

func (x *GetQueryResultsRequest) SetJobId(v string) {
	x.xxx_hidden_JobId = v
}

func (x *GetQueryResultsRequest) SetStartIndex(v *wrapperspb.UInt64Value) {
	x.xxx_hidden_StartIndex = v
}

func (x *GetQueryResultsRequest) SetPageToken(v string) {
	x.xxx_hidden_PageToken = v
}

func (x *GetQueryResultsRequest) SetMaxResults(v *wrapperspb.UInt32Value) {
	x.xxx_hidden_MaxResults = v
}

func (x *GetQueryResultsRequest) SetTimeoutMs(v *wrapperspb.UInt32Value) {
	x.xxx_hidden_TimeoutMs = v
}

func (x *GetQueryResultsRequest) SetLocation(v string) {
	x.xxx_hidden_Location = v
}

func (x *GetQueryResultsRequest) SetFormatOptions(v *DataFormatOptions) {
	x.xxx_hidden_FormatOptions = v
}

func (x *GetQueryResultsRequest) HasStartIndex() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_StartIndex != nil
}

func (x *GetQueryResultsRequest) HasMaxResults() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_MaxResults != nil
}

func (x *GetQueryResultsRequest) HasTimeoutMs() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_TimeoutMs != nil
}

func (x *GetQueryResultsRequest) HasFormatOptions() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_FormatOptions != nil
}

func (x *GetQueryResultsRequest) ClearStartIndex() {
	x.xxx_hidden_StartIndex = nil
}

func (x *GetQueryResultsRequest) ClearMaxResults() {
	x.xxx_hidden_MaxResults = nil
}

func (x *GetQueryResultsRequest) ClearTimeoutMs() {
	x.xxx_hidden_TimeoutMs = nil
}

func (x *GetQueryResultsRequest) ClearFormatOptions() {
	x.xxx_hidden_FormatOptions = nil
}

type GetQueryResultsRequest_builder struct {
	_ [0]func() // Prevents comparability and use of unkeyed literals for the builder.

	// Required. Project ID of the query job.
	ProjectId string
	// Required. Job ID of the query job.
	JobId string
	// Zero-based index of the starting row.
	StartIndex *wrapperspb.UInt64Value
	// Page token, returned by a previous call, to request the next page of
	// results.
	PageToken string
	// Maximum number of results to read.
	MaxResults *wrapperspb.UInt32Value
	// Optional: Specifies the maximum amount of time, in milliseconds, that the
	// client is willing to wait for the query to complete. By default, this limit
	// is 10 seconds (10,000 milliseconds). If the query is complete, the
	// jobComplete field in the response is true. If the query has not yet
	// completed, jobComplete is false.
	//
	// You can request a longer timeout period in the timeoutMs field.  However,
	// the call is not guaranteed to wait for the specified timeout; it typically
	// returns after around 200 seconds (200,000 milliseconds), even if the query
	// is not complete.
	//
	// If jobComplete is false, you can continue to wait for the query to complete
	// by calling the getQueryResults method until the jobComplete field in the
	// getQueryResults response is true.
	TimeoutMs *wrapperspb.UInt32Value
	// The geographic location of the job. You must specify the location to run
	// the job for the following scenarios:
	//
	//   - If the location to run a job is not in the `us` or
	//     the `eu` multi-regional location
	//   - If the job's location is in a single region (for example,
	//
	// `us-central1`)
	//
	// For more information, see how to
	// [specify
	// locations](https://cloud.google.com/bigquery/docs/locations#specify_locations).
	Location string
	// Optional. Output format adjustments.
	FormatOptions *DataFormatOptions
}

func (b0 GetQueryResultsRequest_builder) Build() *GetQueryResultsRequest {
	m0 := &GetQueryResultsRequest{}
	b, x := &b0, m0
	_, _ = b, x
	x.xxx_hidden_ProjectId = b.ProjectId
	x.xxx_hidden_JobId = b.JobId
	x.xxx_hidden_StartIndex = b.StartIndex
	x.xxx_hidden_PageToken = b.PageToken
	x.xxx_hidden_MaxResults = b.MaxResults
	x.xxx_hidden_TimeoutMs = b.TimeoutMs
	x.xxx_hidden_Location = b.Location
	x.xxx_hidden_FormatOptions = b.FormatOptions
	return m0
}

// Response object of GetQueryResults.
type GetQueryResultsResponse struct {
	state                          protoimpl.MessageState  `protogen:"opaque.v1"`
	xxx_hidden_Kind                string                  `protobuf:"bytes,1,opt,name=kind,proto3"`
	xxx_hidden_Etag                string                  `protobuf:"bytes,2,opt,name=etag,proto3"`
	xxx_hidden_Schema              *TableSchema            `protobuf:"bytes,3,opt,name=schema,proto3"`
	xxx_hidden_JobReference        *JobReference           `protobuf:"bytes,4,opt,name=job_reference,json=jobReference,proto3"`
	xxx_hidden_TotalRows           *wrapperspb.UInt64Value `protobuf:"bytes,5,opt,name=total_rows,json=totalRows,proto3"`
	xxx_hidden_PageToken           string                  `protobuf:"bytes,6,opt,name=page_token,json=pageToken,proto3"`
	xxx_hidden_Rows                *[]*structpb.Struct     `protobuf:"bytes,7,rep,name=rows,proto3"`
	xxx_hidden_TotalBytesProcessed *wrapperspb.Int64Value  `protobuf:"bytes,8,opt,name=total_bytes_processed,json=totalBytesProcessed,proto3"`
	xxx_hidden_JobComplete         *wrapperspb.BoolValue   `protobuf:"bytes,9,opt,name=job_complete,json=jobComplete,proto3"`
	xxx_hidden_Errors              *[]*ErrorProto          `protobuf:"bytes,10,rep,name=errors,proto3"`
	xxx_hidden_CacheHit            *wrapperspb.BoolValue   `protobuf:"bytes,11,opt,name=cache_hit,json=cacheHit,proto3"`
	xxx_hidden_NumDmlAffectedRows  *wrapperspb.Int64Value  `protobuf:"bytes,12,opt,name=num_dml_affected_rows,json=numDmlAffectedRows,proto3"`
	unknownFields                  protoimpl.UnknownFields
	sizeCache                      protoimpl.SizeCache
}

func (x *GetQueryResultsResponse) Reset() {
	*x = GetQueryResultsResponse{}
	mi := &file_google_cloud_bigquery_v2_job_proto_msgTypes[10]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetQueryResultsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetQueryResultsResponse) ProtoMessage() {}

func (x *GetQueryResultsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_google_cloud_bigquery_v2_job_proto_msgTypes[10]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

func (x *GetQueryResultsResponse) GetKind() string {
	if x != nil {
		return x.xxx_hidden_Kind
	}
	return ""
}

func (x *GetQueryResultsResponse) GetEtag() string {
	if x != nil {
		return x.xxx_hidden_Etag
	}
	return ""
}

func (x *GetQueryResultsResponse) GetSchema() *TableSchema {
	if x != nil {
		return x.xxx_hidden_Schema
	}
	return nil
}

func (x *GetQueryResultsResponse) GetJobReference() *JobReference {
	if x != nil {
		return x.xxx_hidden_JobReference
	}
	return nil
}

func (x *GetQueryResultsResponse) GetTotalRows() *wrapperspb.UInt64Value {
	if x != nil {
		return x.xxx_hidden_TotalRows
	}
	return nil
}

func (x *GetQueryResultsResponse) GetPageToken() string {
	if x != nil {
		return x.xxx_hidden_PageToken
	}
	return ""
}

func (x *GetQueryResultsResponse) GetRows() []*structpb.Struct {
	if x != nil {
		if x.xxx_hidden_Rows != nil {
			return *x.xxx_hidden_Rows
		}
	}
	return nil
}

func (x *GetQueryResultsResponse) GetTotalBytesProcessed() *wrapperspb.Int64Value {
	if x != nil {
		return x.xxx_hidden_TotalBytesProcessed
	}
	return nil
}

func (x *GetQueryResultsResponse) GetJobComplete() *wrapperspb.BoolValue {
	if x != nil {
		return x.xxx_hidden_JobComplete
	}
	return nil
}

func (x *GetQueryResultsResponse) GetErrors() []*ErrorProto {
	if x != nil {
		if x.xxx_hidden_Errors != nil {
			return *x.xxx_hidden_Errors
		}
	}
	return nil
}

func (x *GetQueryResultsResponse) GetCacheHit() *wrapperspb.BoolValue {
	if x != nil {
		return x.xxx_hidden_CacheHit
	}
	return nil
}

func (x *GetQueryResultsResponse) GetNumDmlAffectedRows() *wrapperspb.Int64Value {
	if x != nil {
		return x.xxx_hidden_NumDmlAffectedRows
	}
	return nil
}

func (x *GetQueryResultsResponse) SetKind(v string) {
	x.xxx_hidden_Kind = v
}

func (x *GetQueryResultsResponse) SetEtag(v string) {
	x.xxx_hidden_Etag = v
}

func (x *GetQueryResultsResponse) SetSchema(v *TableSchema) {
	x.xxx_hidden_Schema = v
}

func (x *GetQueryResultsResponse) SetJobReference(v *JobReference) {
	x.xxx_hidden_JobReference = v
}

func (x *GetQueryResultsResponse) SetTotalRows(v *wrapperspb.UInt64Value) {
	x.xxx_hidden_TotalRows = v
}

func (x *GetQueryResultsResponse) SetPageToken(v string) {
	x.xxx_hidden_PageToken = v
}

func (x *GetQueryResultsResponse) SetRows(v []*structpb.Struct) {
	x.xxx_hidden_Rows = &v
}

func (x *GetQueryResultsResponse) SetTotalBytesProcessed(v *wrapperspb.Int64Value) {
	x.xxx_hidden_TotalBytesProcessed = v
}

func (x *GetQueryResultsResponse) SetJobComplete(v *wrapperspb.BoolValue) {
	x.xxx_hidden_JobComplete = v
}

func (x *GetQueryResultsResponse) SetErrors(v []*ErrorProto) {
	x.xxx_hidden_Errors = &v
}

func (x *GetQueryResultsResponse) SetCacheHit(v *wrapperspb.BoolValue) {
	x.xxx_hidden_CacheHit = v
}

func (x *GetQueryResultsResponse) SetNumDmlAffectedRows(v *wrapperspb.Int64Value) {
	x.xxx_hidden_NumDmlAffectedRows = v
}

func (x *GetQueryResultsResponse) HasSchema() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_Schema != nil
}

func (x *GetQueryResultsResponse) HasJobReference() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_JobReference != nil
}

func (x *GetQueryResultsResponse) HasTotalRows() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_TotalRows != nil
}

func (x *GetQueryResultsResponse) HasTotalBytesProcessed() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_TotalBytesProcessed != nil
}

func (x *GetQueryResultsResponse) HasJobComplete() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_JobComplete != nil
}

func (x *GetQueryResultsResponse) HasCacheHit() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_CacheHit != nil
}

func (x *GetQueryResultsResponse) HasNumDmlAffectedRows() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_NumDmlAffectedRows != nil
}

func (x *GetQueryResultsResponse) ClearSchema() {
	x.xxx_hidden_Schema = nil
}

func (x *GetQueryResultsResponse) ClearJobReference() {
	x.xxx_hidden_JobReference = nil
}

func (x *GetQueryResultsResponse) ClearTotalRows() {
	x.xxx_hidden_TotalRows = nil
}

func (x *GetQueryResultsResponse) ClearTotalBytesProcessed() {
	x.xxx_hidden_TotalBytesProcessed = nil
}

func (x *GetQueryResultsResponse) ClearJobComplete() {
	x.xxx_hidden_JobComplete = nil
}

func (x *GetQueryResultsResponse) ClearCacheHit() {
	x.xxx_hidden_CacheHit = nil
}

func (x *GetQueryResultsResponse) ClearNumDmlAffectedRows() {
	x.xxx_hidden_NumDmlAffectedRows = nil
}

type GetQueryResultsResponse_builder struct {
	_ [0]func() // Prevents comparability and use of unkeyed literals for the builder.

	// The resource type of the response.
	Kind string
	// A hash of this response.
	Etag string
	// The schema of the results. Present only when the query completes
	// successfully.
	Schema *TableSchema
	// Reference to the BigQuery Job that was created to run the query. This field
	// will be present even if the original request timed out, in which case
	// GetQueryResults can be used to read the results once the query has
	// completed. Since this API only returns the first page of results,
	// subsequent pages can be fetched via the same mechanism (GetQueryResults).
	JobReference *JobReference
	// The total number of rows in the complete query result set, which can be
	// more than the number of rows in this single page of results. Present only
	// when the query completes successfully.
	TotalRows *wrapperspb.UInt64Value
	// A token used for paging results.  When this token is non-empty, it
	// indicates additional results are available.
	PageToken string
	// An object with as many results as can be contained within the maximum
	// permitted reply size. To get any additional rows, you can call
	// GetQueryResults and specify the jobReference returned above. Present only
	// when the query completes successfully.
	//
	// The REST-based representation of this data leverages a series of
	// JSON f,v objects for indicating fields and values.
	Rows []*structpb.Struct
	// The total number of bytes processed for this query.
	TotalBytesProcessed *wrapperspb.Int64Value
	// Whether the query has completed or not. If rows or totalRows are present,
	// this will always be true. If this is false, totalRows will not be
	// available.
	JobComplete *wrapperspb.BoolValue
	// Output only. The first errors or warnings encountered during the running
	// of the job. The final message includes the number of errors that caused the
	// process to stop. Errors here do not necessarily mean that the job has
	// completed or was unsuccessful. For more information about error messages,
	// see [Error
	// messages](https://cloud.google.com/bigquery/docs/error-messages).
	Errors []*ErrorProto
	// Whether the query result was fetched from the query cache.
	CacheHit *wrapperspb.BoolValue
	// Output only. The number of rows affected by a DML statement. Present only
	// for DML statements INSERT, UPDATE or DELETE.
	NumDmlAffectedRows *wrapperspb.Int64Value
}

func (b0 GetQueryResultsResponse_builder) Build() *GetQueryResultsResponse {
	m0 := &GetQueryResultsResponse{}
	b, x := &b0, m0
	_, _ = b, x
	x.xxx_hidden_Kind = b.Kind
	x.xxx_hidden_Etag = b.Etag
	x.xxx_hidden_Schema = b.Schema
	x.xxx_hidden_JobReference = b.JobReference
	x.xxx_hidden_TotalRows = b.TotalRows
	x.xxx_hidden_PageToken = b.PageToken
	x.xxx_hidden_Rows = &b.Rows
	x.xxx_hidden_TotalBytesProcessed = b.TotalBytesProcessed
	x.xxx_hidden_JobComplete = b.JobComplete
	x.xxx_hidden_Errors = &b.Errors
	x.xxx_hidden_CacheHit = b.CacheHit
	x.xxx_hidden_NumDmlAffectedRows = b.NumDmlAffectedRows
	return m0
}

// Request format for the query request.
type PostQueryRequest struct {
	state                   protoimpl.MessageState `protogen:"opaque.v1"`
	xxx_hidden_ProjectId    string                 `protobuf:"bytes,1,opt,name=project_id,json=projectId,proto3"`
	xxx_hidden_QueryRequest *QueryRequest          `protobuf:"bytes,2,opt,name=query_request,json=queryRequest,proto3"`
	unknownFields           protoimpl.UnknownFields
	sizeCache               protoimpl.SizeCache
}

func (x *PostQueryRequest) Reset() {
	*x = PostQueryRequest{}
	mi := &file_google_cloud_bigquery_v2_job_proto_msgTypes[11]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *PostQueryRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*PostQueryRequest) ProtoMessage() {}

func (x *PostQueryRequest) ProtoReflect() protoreflect.Message {
	mi := &file_google_cloud_bigquery_v2_job_proto_msgTypes[11]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

func (x *PostQueryRequest) GetProjectId() string {
	if x != nil {
		return x.xxx_hidden_ProjectId
	}
	return ""
}

func (x *PostQueryRequest) GetQueryRequest() *QueryRequest {
	if x != nil {
		return x.xxx_hidden_QueryRequest
	}
	return nil
}

func (x *PostQueryRequest) SetProjectId(v string) {
	x.xxx_hidden_ProjectId = v
}

func (x *PostQueryRequest) SetQueryRequest(v *QueryRequest) {
	x.xxx_hidden_QueryRequest = v
}

func (x *PostQueryRequest) HasQueryRequest() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_QueryRequest != nil
}

func (x *PostQueryRequest) ClearQueryRequest() {
	x.xxx_hidden_QueryRequest = nil
}

type PostQueryRequest_builder struct {
	_ [0]func() // Prevents comparability and use of unkeyed literals for the builder.

	// Required. Project ID of the query request.
	ProjectId string
	// The query request body.
	QueryRequest *QueryRequest
}

func (b0 PostQueryRequest_builder) Build() *PostQueryRequest {
	m0 := &PostQueryRequest{}
	b, x := &b0, m0
	_, _ = b, x
	x.xxx_hidden_ProjectId = b.ProjectId
	x.xxx_hidden_QueryRequest = b.QueryRequest
	return m0
}

// Describes the format of the jobs.query request.
type QueryRequest struct {
	state                                         protoimpl.MessageState                           `protogen:"opaque.v1"`
	xxx_hidden_Kind                               string                                           `protobuf:"bytes,2,opt,name=kind,proto3"`
	xxx_hidden_Query                              string                                           `protobuf:"bytes,3,opt,name=query,proto3"`
	xxx_hidden_MaxResults                         *wrapperspb.UInt32Value                          `protobuf:"bytes,4,opt,name=max_results,json=maxResults,proto3"`
	xxx_hidden_DefaultDataset                     *DatasetReference                                `protobuf:"bytes,5,opt,name=default_dataset,json=defaultDataset,proto3"`
	xxx_hidden_TimeoutMs                          *wrapperspb.UInt32Value                          `protobuf:"bytes,6,opt,name=timeout_ms,json=timeoutMs,proto3"`
	xxx_hidden_JobTimeoutMs                       int64                                            `protobuf:"varint,26,opt,name=job_timeout_ms,json=jobTimeoutMs,proto3,oneof"`
	xxx_hidden_MaxSlots                           int32                                            `protobuf:"varint,28,opt,name=max_slots,json=maxSlots,proto3,oneof"`
	xxx_hidden_DestinationEncryptionConfiguration *EncryptionConfiguration                         `protobuf:"bytes,27,opt,name=destination_encryption_configuration,json=destinationEncryptionConfiguration,proto3"`
	xxx_hidden_DryRun                             bool                                             `protobuf:"varint,7,opt,name=dry_run,json=dryRun,proto3"`
	xxx_hidden_UseQueryCache                      *wrapperspb.BoolValue                            `protobuf:"bytes,9,opt,name=use_query_cache,json=useQueryCache,proto3"`
	xxx_hidden_UseLegacySql                       *wrapperspb.BoolValue                            `protobuf:"bytes,10,opt,name=use_legacy_sql,json=useLegacySql,proto3"`
	xxx_hidden_ParameterMode                      string                                           `protobuf:"bytes,11,opt,name=parameter_mode,json=parameterMode,proto3"`
	xxx_hidden_QueryParameters                    *[]*QueryParameter                               `protobuf:"bytes,12,rep,name=query_parameters,json=queryParameters,proto3"`
	xxx_hidden_Location                           string                                           `protobuf:"bytes,13,opt,name=location,proto3"`
	xxx_hidden_FormatOptions                      *DataFormatOptions                               `protobuf:"bytes,15,opt,name=format_options,json=formatOptions,proto3"`
	xxx_hidden_ConnectionProperties               *[]*ConnectionProperty                           `protobuf:"bytes,16,rep,name=connection_properties,json=connectionProperties,proto3"`
	xxx_hidden_Labels                             map[string]string                                `protobuf:"bytes,17,rep,name=labels,proto3" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	xxx_hidden_MaximumBytesBilled                 *wrapperspb.Int64Value                           `protobuf:"bytes,18,opt,name=maximum_bytes_billed,json=maximumBytesBilled,proto3"`
	xxx_hidden_RequestId                          string                                           `protobuf:"bytes,19,opt,name=request_id,json=requestId,proto3"`
	xxx_hidden_CreateSession                      *wrapperspb.BoolValue                            `protobuf:"bytes,20,opt,name=create_session,json=createSession,proto3"`
	xxx_hidden_JobCreationMode                    QueryRequest_JobCreationMode                     `protobuf:"varint,22,opt,name=job_creation_mode,json=jobCreationMode,proto3,enum=google.cloud.bigquery.v2.QueryRequest_JobCreationMode"`
	xxx_hidden_Reservation                        *string                                          `protobuf:"bytes,24,opt,name=reservation,proto3,oneof"`
	xxx_hidden_WriteIncrementalResults            bool                                             `protobuf:"varint,25,opt,name=write_incremental_results,json=writeIncrementalResults,proto3"`
	xxx_hidden_QueryResultsFormat                 QueryRequest_QueryResultsFormat                  `protobuf:"varint,29,opt,name=query_results_format,json=queryResultsFormat,proto3,enum=google.cloud.bigquery.v2.QueryRequest_QueryResultsFormat"`
	xxx_hidden_ResultsFormatSerializationOptions  isQueryRequest_ResultsFormatSerializationOptions `protobuf_oneof:"results_format_serialization_options"`
	XXX_raceDetectHookData                        protoimpl.RaceDetectHookData
	XXX_presence                                  [1]uint32
	unknownFields                                 protoimpl.UnknownFields
	sizeCache                                     protoimpl.SizeCache
}

func (x *QueryRequest) Reset() {
	*x = QueryRequest{}
	mi := &file_google_cloud_bigquery_v2_job_proto_msgTypes[12]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *QueryRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*QueryRequest) ProtoMessage() {}

func (x *QueryRequest) ProtoReflect() protoreflect.Message {
	mi := &file_google_cloud_bigquery_v2_job_proto_msgTypes[12]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

func (x *QueryRequest) GetKind() string {
	if x != nil {
		return x.xxx_hidden_Kind
	}
	return ""
}

func (x *QueryRequest) GetQuery() string {
	if x != nil {
		return x.xxx_hidden_Query
	}
	return ""
}

func (x *QueryRequest) GetMaxResults() *wrapperspb.UInt32Value {
	if x != nil {
		return x.xxx_hidden_MaxResults
	}
	return nil
}

func (x *QueryRequest) GetDefaultDataset() *DatasetReference {
	if x != nil {
		return x.xxx_hidden_DefaultDataset
	}
	return nil
}

func (x *QueryRequest) GetTimeoutMs() *wrapperspb.UInt32Value {
	if x != nil {
		return x.xxx_hidden_TimeoutMs
	}
	return nil
}

func (x *QueryRequest) GetJobTimeoutMs() int64 {
	if x != nil {
		return x.xxx_hidden_JobTimeoutMs
	}
	return 0
}

func (x *QueryRequest) GetMaxSlots() int32 {
	if x != nil {
		return x.xxx_hidden_MaxSlots
	}
	return 0
}

func (x *QueryRequest) GetDestinationEncryptionConfiguration() *EncryptionConfiguration {
	if x != nil {
		return x.xxx_hidden_DestinationEncryptionConfiguration
	}
	return nil
}

func (x *QueryRequest) GetDryRun() bool {
	if x != nil {
		return x.xxx_hidden_DryRun
	}
	return false
}

func (x *QueryRequest) GetUseQueryCache() *wrapperspb.BoolValue {
	if x != nil {
		return x.xxx_hidden_UseQueryCache
	}
	return nil
}

func (x *QueryRequest) GetUseLegacySql() *wrapperspb.BoolValue {
	if x != nil {
		return x.xxx_hidden_UseLegacySql
	}
	return nil
}

func (x *QueryRequest) GetParameterMode() string {
	if x != nil {
		return x.xxx_hidden_ParameterMode
	}
	return ""
}

func (x *QueryRequest) GetQueryParameters() []*QueryParameter {
	if x != nil {
		if x.xxx_hidden_QueryParameters != nil {
			return *x.xxx_hidden_QueryParameters
		}
	}
	return nil
}

func (x *QueryRequest) GetLocation() string {
	if x != nil {
		return x.xxx_hidden_Location
	}
	return ""
}

func (x *QueryRequest) GetFormatOptions() *DataFormatOptions {
	if x != nil {
		return x.xxx_hidden_FormatOptions
	}
	return nil
}

func (x *QueryRequest) GetConnectionProperties() []*ConnectionProperty {
	if x != nil {
		if x.xxx_hidden_ConnectionProperties != nil {
			return *x.xxx_hidden_ConnectionProperties
		}
	}
	return nil
}

func (x *QueryRequest) GetLabels() map[string]string {
	if x != nil {
		return x.xxx_hidden_Labels
	}
	return nil
}

func (x *QueryRequest) GetMaximumBytesBilled() *wrapperspb.Int64Value {
	if x != nil {
		return x.xxx_hidden_MaximumBytesBilled
	}
	return nil
}

func (x *QueryRequest) GetRequestId() string {
	if x != nil {
		return x.xxx_hidden_RequestId
	}
	return ""
}

func (x *QueryRequest) GetCreateSession() *wrapperspb.BoolValue {
	if x != nil {
		return x.xxx_hidden_CreateSession
	}
	return nil
}

func (x *QueryRequest) GetJobCreationMode() QueryRequest_JobCreationMode {
	if x != nil {
		return x.xxx_hidden_JobCreationMode
	}
	return QueryRequest_JOB_CREATION_MODE_UNSPECIFIED
}

func (x *QueryRequest) GetReservation() string {
	if x != nil {
		if x.xxx_hidden_Reservation != nil {
			return *x.xxx_hidden_Reservation
		}
		return ""
	}
	return ""
}

func (x *QueryRequest) GetWriteIncrementalResults() bool {
	if x != nil {
		return x.xxx_hidden_WriteIncrementalResults
	}
	return false
}

func (x *QueryRequest) GetQueryResultsFormat() QueryRequest_QueryResultsFormat {
	if x != nil {
		return x.xxx_hidden_QueryResultsFormat
	}
	return QueryRequest_QUERY_RESULTS_FORMAT_UNSPECIFIED
}

func (x *QueryRequest) GetArrowSerializationOptions() *ArrowSerializationOptions {
	if x != nil {
		if x, ok := x.xxx_hidden_ResultsFormatSerializationOptions.(*queryRequest_ArrowSerializationOptions); ok {
			return x.ArrowSerializationOptions
		}
	}
	return nil
}

func (x *QueryRequest) SetKind(v string) {
	x.xxx_hidden_Kind = v
}

func (x *QueryRequest) SetQuery(v string) {
	x.xxx_hidden_Query = v
}

func (x *QueryRequest) SetMaxResults(v *wrapperspb.UInt32Value) {
	x.xxx_hidden_MaxResults = v
}

func (x *QueryRequest) SetDefaultDataset(v *DatasetReference) {
	x.xxx_hidden_DefaultDataset = v
}

func (x *QueryRequest) SetTimeoutMs(v *wrapperspb.UInt32Value) {
	x.xxx_hidden_TimeoutMs = v
}

func (x *QueryRequest) SetJobTimeoutMs(v int64) {
	x.xxx_hidden_JobTimeoutMs = v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 5, 25)
}

func (x *QueryRequest) SetMaxSlots(v int32) {
	x.xxx_hidden_MaxSlots = v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 6, 25)
}

func (x *QueryRequest) SetDestinationEncryptionConfiguration(v *EncryptionConfiguration) {
	x.xxx_hidden_DestinationEncryptionConfiguration = v
}

func (x *QueryRequest) SetDryRun(v bool) {
	x.xxx_hidden_DryRun = v
}

func (x *QueryRequest) SetUseQueryCache(v *wrapperspb.BoolValue) {
	x.xxx_hidden_UseQueryCache = v
}

func (x *QueryRequest) SetUseLegacySql(v *wrapperspb.BoolValue) {
	x.xxx_hidden_UseLegacySql = v
}

func (x *QueryRequest) SetParameterMode(v string) {
	x.xxx_hidden_ParameterMode = v
}

func (x *QueryRequest) SetQueryParameters(v []*QueryParameter) {
	x.xxx_hidden_QueryParameters = &v
}

func (x *QueryRequest) SetLocation(v string) {
	x.xxx_hidden_Location = v
}

func (x *QueryRequest) SetFormatOptions(v *DataFormatOptions) {
	x.xxx_hidden_FormatOptions = v
}

func (x *QueryRequest) SetConnectionProperties(v []*ConnectionProperty) {
	x.xxx_hidden_ConnectionProperties = &v
}

func (x *QueryRequest) SetLabels(v map[string]string) {
	x.xxx_hidden_Labels = v
}

func (x *QueryRequest) SetMaximumBytesBilled(v *wrapperspb.Int64Value) {
	x.xxx_hidden_MaximumBytesBilled = v
}

func (x *QueryRequest) SetRequestId(v string) {
	x.xxx_hidden_RequestId = v
}

func (x *QueryRequest) SetCreateSession(v *wrapperspb.BoolValue) {
	x.xxx_hidden_CreateSession = v
}

func (x *QueryRequest) SetJobCreationMode(v QueryRequest_JobCreationMode) {
	x.xxx_hidden_JobCreationMode = v
}

func (x *QueryRequest) SetReservation(v string) {
	x.xxx_hidden_Reservation = &v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 21, 25)
}

func (x *QueryRequest) SetWriteIncrementalResults(v bool) {
	x.xxx_hidden_WriteIncrementalResults = v
}

func (x *QueryRequest) SetQueryResultsFormat(v QueryRequest_QueryResultsFormat) {
	x.xxx_hidden_QueryResultsFormat = v
}

func (x *QueryRequest) SetArrowSerializationOptions(v *ArrowSerializationOptions) {
	if v == nil {
		x.xxx_hidden_ResultsFormatSerializationOptions = nil
		return
	}
	x.xxx_hidden_ResultsFormatSerializationOptions = &queryRequest_ArrowSerializationOptions{v}
}

func (x *QueryRequest) HasMaxResults() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_MaxResults != nil
}

func (x *QueryRequest) HasDefaultDataset() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_DefaultDataset != nil
}

func (x *QueryRequest) HasTimeoutMs() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_TimeoutMs != nil
}

func (x *QueryRequest) HasJobTimeoutMs() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 5)
}

func (x *QueryRequest) HasMaxSlots() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 6)
}

func (x *QueryRequest) HasDestinationEncryptionConfiguration() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_DestinationEncryptionConfiguration != nil
}

func (x *QueryRequest) HasUseQueryCache() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_UseQueryCache != nil
}

func (x *QueryRequest) HasUseLegacySql() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_UseLegacySql != nil
}

func (x *QueryRequest) HasFormatOptions() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_FormatOptions != nil
}

func (x *QueryRequest) HasMaximumBytesBilled() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_MaximumBytesBilled != nil
}

func (x *QueryRequest) HasCreateSession() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_CreateSession != nil
}

func (x *QueryRequest) HasReservation() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 21)
}

func (x *QueryRequest) HasResultsFormatSerializationOptions() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_ResultsFormatSerializationOptions != nil
}

func (x *QueryRequest) HasArrowSerializationOptions() bool {
	if x == nil {
		return false
	}
	_, ok := x.xxx_hidden_ResultsFormatSerializationOptions.(*queryRequest_ArrowSerializationOptions)
	return ok
}

func (x *QueryRequest) ClearMaxResults() {
	x.xxx_hidden_MaxResults = nil
}

func (x *QueryRequest) ClearDefaultDataset() {
	x.xxx_hidden_DefaultDataset = nil
}

func (x *QueryRequest) ClearTimeoutMs() {
	x.xxx_hidden_TimeoutMs = nil
}

func (x *QueryRequest) ClearJobTimeoutMs() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 5)
	x.xxx_hidden_JobTimeoutMs = 0
}

func (x *QueryRequest) ClearMaxSlots() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 6)
	x.xxx_hidden_MaxSlots = 0
}

func (x *QueryRequest) ClearDestinationEncryptionConfiguration() {
	x.xxx_hidden_DestinationEncryptionConfiguration = nil
}

func (x *QueryRequest) ClearUseQueryCache() {
	x.xxx_hidden_UseQueryCache = nil
}

func (x *QueryRequest) ClearUseLegacySql() {
	x.xxx_hidden_UseLegacySql = nil
}

func (x *QueryRequest) ClearFormatOptions() {
	x.xxx_hidden_FormatOptions = nil
}

func (x *QueryRequest) ClearMaximumBytesBilled() {
	x.xxx_hidden_MaximumBytesBilled = nil
}

func (x *QueryRequest) ClearCreateSession() {
	x.xxx_hidden_CreateSession = nil
}

func (x *QueryRequest) ClearReservation() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 21)
	x.xxx_hidden_Reservation = nil
}

func (x *QueryRequest) ClearResultsFormatSerializationOptions() {
	x.xxx_hidden_ResultsFormatSerializationOptions = nil
}

func (x *QueryRequest) ClearArrowSerializationOptions() {
	if _, ok := x.xxx_hidden_ResultsFormatSerializationOptions.(*queryRequest_ArrowSerializationOptions); ok {
		x.xxx_hidden_ResultsFormatSerializationOptions = nil
	}
}

const QueryRequest_ResultsFormatSerializationOptions_not_set_case case_QueryRequest_ResultsFormatSerializationOptions = 0
const QueryRequest_ArrowSerializationOptions_case case_QueryRequest_ResultsFormatSerializationOptions = 30

func (x *QueryRequest) WhichResultsFormatSerializationOptions() case_QueryRequest_ResultsFormatSerializationOptions {
	if x == nil {
		return QueryRequest_ResultsFormatSerializationOptions_not_set_case
	}
	switch x.xxx_hidden_ResultsFormatSerializationOptions.(type) {
	case *queryRequest_ArrowSerializationOptions:
		return QueryRequest_ArrowSerializationOptions_case
	default:
		return QueryRequest_ResultsFormatSerializationOptions_not_set_case
	}
}

type QueryRequest_builder struct {
	_ [0]func() // Prevents comparability and use of unkeyed literals for the builder.

	// The resource type of the request.
	Kind string
	// Required. A query string to execute, using Google Standard SQL or legacy
	// SQL syntax. Example: "SELECT COUNT(f1) FROM
	// myProjectId.myDatasetId.myTableId".
	Query string
	// Optional. The maximum number of rows of data to return per page of
	// results. Setting this flag to a small value such as 1000 and then paging
	// through results might improve reliability when the query result set is
	// large. In addition to this limit, responses are also limited to 10 MB. By
	// default, there is no maximum row count, and only the byte limit applies.
	MaxResults *wrapperspb.UInt32Value
	// Optional. Specifies the default datasetId and projectId to assume for any
	// unqualified table names in the query. If not set, all table names in the
	// query string must be qualified in the format 'datasetId.tableId'.
	DefaultDataset *DatasetReference
	// Optional. Optional: Specifies the maximum amount of time, in milliseconds,
	// that the client is willing to wait for the query to complete. By default,
	// this limit is 10 seconds (10,000 milliseconds). If the query is complete,
	// the jobComplete field in the response is true. If the query has not yet
	// completed, jobComplete is false.
	//
	// You can request a longer timeout period in the timeoutMs field.  However,
	// the call is not guaranteed to wait for the specified timeout; it typically
	// returns after around 200 seconds (200,000 milliseconds), even if the query
	// is not complete.
	//
	// If jobComplete is false, you can continue to wait for the query to complete
	// by calling the getQueryResults method until the jobComplete field in the
	// getQueryResults response is true.
	TimeoutMs *wrapperspb.UInt32Value
	// Optional. Job timeout in milliseconds. If this time limit is exceeded,
	// BigQuery will attempt to stop a longer job, but may not always succeed in
	// canceling it before the job completes. For example, a job that takes more
	// than 60 seconds to complete has a better chance of being stopped than a job
	// that takes 10 seconds to complete. This timeout applies to the query even
	// if a job does not need to be created.
	JobTimeoutMs *int64
	// Optional. A target limit on the rate of slot consumption by this query. If
	// set to a value > 0, BigQuery will attempt to limit the rate of slot
	// consumption by this query to keep it below the configured limit, even if
	// the query is eligible for more slots based on fair scheduling. The unused
	// slots will be available for other jobs and queries to use.
	//
	// Note: This feature is not yet generally available.
	MaxSlots *int32
	// Optional. Custom encryption configuration (e.g., Cloud KMS keys)
	DestinationEncryptionConfiguration *EncryptionConfiguration
	// Optional. If set to true, BigQuery doesn't run the job. Instead, if the
	// query is valid, BigQuery returns statistics about the job such as how many
	// bytes would be processed. If the query is invalid, an error returns. The
	// default value is false.
	DryRun bool
	// Optional. Whether to look for the result in the query cache. The query
	// cache is a best-effort cache that will be flushed whenever tables in the
	// query are modified. The default value is true.
	UseQueryCache *wrapperspb.BoolValue
	// Specifies whether to use BigQuery's legacy SQL dialect for this query. The
	// default value is true. If set to false, the query uses BigQuery's
	// [GoogleSQL](https://docs.cloud.google.com/bigquery/docs/introduction-sql).
	// When useLegacySql is set to false, the value of flattenResults is ignored;
	// query will be run as if flattenResults is false.
	UseLegacySql *wrapperspb.BoolValue
	// GoogleSQL only. Set to POSITIONAL to use positional (?) query parameters
	// or to NAMED to use named (@myparam) query parameters in this query.
	ParameterMode string
	// Query parameters for GoogleSQL queries.
	QueryParameters []*QueryParameter
	// The geographic location where the job should run.
	// For more information, see how to
	// [specify
	// locations](https://cloud.google.com/bigquery/docs/locations#specify_locations).
	Location string
	// Optional. Output format adjustments.
	FormatOptions *DataFormatOptions
	// Optional. Connection properties which can modify the query behavior.
	ConnectionProperties []*ConnectionProperty
	// Optional. The labels associated with this query.
	// Labels can be used to organize and group query jobs.
	// Label keys and values can be no longer than 63 characters, can only contain
	// lowercase letters, numeric characters, underscores and dashes.
	// International characters are allowed. Label keys must start with a letter
	// and each label in the list must have a different key.
	Labels map[string]string
	// Optional. Limits the bytes billed for this query. Queries with
	// bytes billed above this limit will fail (without incurring a charge).
	// If unspecified, the project default is used.
	MaximumBytesBilled *wrapperspb.Int64Value
	// Optional. A unique user provided identifier to ensure idempotent behavior
	// for queries. Note that this is different from the job_id. It has the
	// following properties:
	//
	//  1. It is case-sensitive, limited to up to 36 ASCII characters. A UUID is
	//     recommended.
	//
	//  2. Read only queries can ignore this token since they are nullipotent by
	//     definition.
	//
	//  3. For the purposes of idempotency ensured by the request_id, a request
	//     is considered duplicate of another only if they have the same request_id
	//     and are actually duplicates. When determining whether a request is a
	//     duplicate of another request, all parameters in the request that
	//     may affect the result are considered. For example, query,
	//     connection_properties, query_parameters, use_legacy_sql are parameters
	//     that affect the result and are considered when determining whether a
	//     request is a duplicate, but properties like timeout_ms don't
	//     affect the result and are thus not considered. Dry run query
	//     requests are never considered duplicate of another request.
	//
	//  4. When a duplicate mutating query request is detected, it returns:
	//     a. the results of the mutation if it completes successfully within
	//     the timeout.
	//     b. the running operation if it is still in progress at the end of the
	//     timeout.
	//
	//  5. Its lifetime is limited to 15 minutes. In other words, if two
	//     requests are sent with the same request_id, but more than 15 minutes
	//     apart, idempotency is not guaranteed.
	RequestId string
	// Optional. If true, creates a new session using a randomly generated
	// session_id. If false, runs query with an existing session_id passed in
	// ConnectionProperty, otherwise runs query in non-session mode.
	//
	// The session location will be set to QueryRequest.location if it is present,
	// otherwise it's set to the default location based on existing routing logic.
	CreateSession *wrapperspb.BoolValue
	// Optional. If not set, jobs are always required.
	//
	// If set, the query request will follow the behavior described
	// JobCreationMode.
	JobCreationMode QueryRequest_JobCreationMode
	// Optional. The reservation that jobs.query request would use. User can
	// specify a reservation to execute the job.query. The expected format is
	// `projects/{project}/locations/{location}/reservations/{reservation}`.
	// Forces the query to use on-demand billing when set to `none`.
	// This requires the project or organization to have
	// `reservation_override_mode` set to `ALLOW_ANY_OVERRIDE`.
	Reservation *string
	// Optional. This is only supported for SELECT query. If set, the query is
	// allowed to write results incrementally to the temporary result table. This
	// may incur a performance penalty. This option cannot be used with Legacy
	// SQL. This feature is not yet available.
	WriteIncrementalResults bool
	// Optional. The query results format.
	// If the value is anything other than `STRUCT_ENCODING` or unspecified:
	//
	//   - The schema of the results will be provided in
	//     `QueryResponse.results_schema` field.
	//   - The results of the first page will be provided in
	//     `QueryResponse.results` field.
	//   - The `QueryResponse.rows` will not be populated.
	//   - The `QueryResponse.schema` for `QueryResponse.rows` will also not be
	//     populated since it is the schema of the `QueryResponse.rows`.
	//
	// This feature is not yet available.
	QueryResultsFormat QueryRequest_QueryResultsFormat
	// Results serialization options for `query_results_format` other than
	// `STRUCT_ENCODING`.
	//
	// This feature is not yet available.

	// Fields of oneof xxx_hidden_ResultsFormatSerializationOptions:
	// Optional. Options specific to the Apache Arrow output format.
	ArrowSerializationOptions *ArrowSerializationOptions
	// -- end of xxx_hidden_ResultsFormatSerializationOptions
}

func (b0 QueryRequest_builder) Build() *QueryRequest {
	m0 := &QueryRequest{}
	b, x := &b0, m0
	_, _ = b, x
	x.xxx_hidden_Kind = b.Kind
	x.xxx_hidden_Query = b.Query
	x.xxx_hidden_MaxResults = b.MaxResults
	x.xxx_hidden_DefaultDataset = b.DefaultDataset
	x.xxx_hidden_TimeoutMs = b.TimeoutMs
	if b.JobTimeoutMs != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 5, 25)
		x.xxx_hidden_JobTimeoutMs = *b.JobTimeoutMs
	}
	if b.MaxSlots != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 6, 25)
		x.xxx_hidden_MaxSlots = *b.MaxSlots
	}
	x.xxx_hidden_DestinationEncryptionConfiguration = b.DestinationEncryptionConfiguration
	x.xxx_hidden_DryRun = b.DryRun
	x.xxx_hidden_UseQueryCache = b.UseQueryCache
	x.xxx_hidden_UseLegacySql = b.UseLegacySql
	x.xxx_hidden_ParameterMode = b.ParameterMode
	x.xxx_hidden_QueryParameters = &b.QueryParameters
	x.xxx_hidden_Location = b.Location
	x.xxx_hidden_FormatOptions = b.FormatOptions
	x.xxx_hidden_ConnectionProperties = &b.ConnectionProperties
	x.xxx_hidden_Labels = b.Labels
	x.xxx_hidden_MaximumBytesBilled = b.MaximumBytesBilled
	x.xxx_hidden_RequestId = b.RequestId
	x.xxx_hidden_CreateSession = b.CreateSession
	x.xxx_hidden_JobCreationMode = b.JobCreationMode
	if b.Reservation != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 21, 25)
		x.xxx_hidden_Reservation = b.Reservation
	}
	x.xxx_hidden_WriteIncrementalResults = b.WriteIncrementalResults
	x.xxx_hidden_QueryResultsFormat = b.QueryResultsFormat
	if b.ArrowSerializationOptions != nil {
		x.xxx_hidden_ResultsFormatSerializationOptions = &queryRequest_ArrowSerializationOptions{b.ArrowSerializationOptions}
	}
	return m0
}

type case_QueryRequest_ResultsFormatSerializationOptions protoreflect.FieldNumber

func (x case_QueryRequest_ResultsFormatSerializationOptions) String() string {
	md := file_google_cloud_bigquery_v2_job_proto_msgTypes[12].Descriptor()
	if x == 0 {
		return "not set"
	}
	return protoimpl.X.MessageFieldStringOf(md, protoreflect.FieldNumber(x))
}

type isQueryRequest_ResultsFormatSerializationOptions interface {
	isQueryRequest_ResultsFormatSerializationOptions()
}

type queryRequest_ArrowSerializationOptions struct {
	// Optional. Options specific to the Apache Arrow output format.
	ArrowSerializationOptions *ArrowSerializationOptions `protobuf:"bytes,30,opt,name=arrow_serialization_options,json=arrowSerializationOptions,proto3,oneof"`
}

func (*queryRequest_ArrowSerializationOptions) isQueryRequest_ResultsFormatSerializationOptions() {}

type QueryResponse struct {
	state                          protoimpl.MessageState        `protogen:"opaque.v1"`
	xxx_hidden_Kind                string                        `protobuf:"bytes,1,opt,name=kind,proto3"`
	xxx_hidden_Schema              *TableSchema                  `protobuf:"bytes,2,opt,name=schema,proto3"`
	xxx_hidden_JobReference        *JobReference                 `protobuf:"bytes,3,opt,name=job_reference,json=jobReference,proto3"`
	xxx_hidden_JobCreationReason   *JobCreationReason            `protobuf:"bytes,15,opt,name=job_creation_reason,json=jobCreationReason,proto3"`
	xxx_hidden_QueryId             string                        `protobuf:"bytes,14,opt,name=query_id,json=queryId,proto3"`
	xxx_hidden_Location            string                        `protobuf:"bytes,18,opt,name=location,proto3"`
	xxx_hidden_TotalRows           *wrapperspb.UInt64Value       `protobuf:"bytes,4,opt,name=total_rows,json=totalRows,proto3"`
	xxx_hidden_PageToken           string                        `protobuf:"bytes,5,opt,name=page_token,json=pageToken,proto3"`
	xxx_hidden_Rows                *[]*structpb.Struct           `protobuf:"bytes,6,rep,name=rows,proto3"`
	xxx_hidden_TotalBytesProcessed *wrapperspb.Int64Value        `protobuf:"bytes,7,opt,name=total_bytes_processed,json=totalBytesProcessed,proto3"`
	xxx_hidden_TotalBytesBilled    int64                         `protobuf:"varint,16,opt,name=total_bytes_billed,json=totalBytesBilled,proto3,oneof"`
	xxx_hidden_TotalSlotMs         int64                         `protobuf:"varint,17,opt,name=total_slot_ms,json=totalSlotMs,proto3,oneof"`
	xxx_hidden_JobComplete         *wrapperspb.BoolValue         `protobuf:"bytes,8,opt,name=job_complete,json=jobComplete,proto3"`
	xxx_hidden_Errors              *[]*ErrorProto                `protobuf:"bytes,9,rep,name=errors,proto3"`
	xxx_hidden_CacheHit            *wrapperspb.BoolValue         `protobuf:"bytes,10,opt,name=cache_hit,json=cacheHit,proto3"`
	xxx_hidden_NumDmlAffectedRows  *wrapperspb.Int64Value        `protobuf:"bytes,11,opt,name=num_dml_affected_rows,json=numDmlAffectedRows,proto3"`
	xxx_hidden_SessionInfo         *SessionInfo                  `protobuf:"bytes,12,opt,name=session_info,json=sessionInfo,proto3"`
	xxx_hidden_DmlStats            *DmlStats                     `protobuf:"bytes,13,opt,name=dml_stats,json=dmlStats,proto3"`
	xxx_hidden_CreationTime        int64                         `protobuf:"varint,19,opt,name=creation_time,json=creationTime,proto3,oneof"`
	xxx_hidden_StartTime           int64                         `protobuf:"varint,20,opt,name=start_time,json=startTime,proto3,oneof"`
	xxx_hidden_EndTime             int64                         `protobuf:"varint,21,opt,name=end_time,json=endTime,proto3,oneof"`
	xxx_hidden_ResultsSchema       isQueryResponse_ResultsSchema `protobuf_oneof:"results_schema"`
	xxx_hidden_Results             isQueryResponse_Results       `protobuf_oneof:"results"`
	xxx_hidden_PageRowCount        int64                         `protobuf:"varint,25,opt,name=page_row_count,json=pageRowCount,proto3"`
	xxx_hidden_StatementType       string                        `protobuf:"bytes,26,opt,name=statement_type,json=statementType,proto3"`
	XXX_raceDetectHookData         protoimpl.RaceDetectHookData
	XXX_presence                   [1]uint32
	unknownFields                  protoimpl.UnknownFields
	sizeCache                      protoimpl.SizeCache
}

func (x *QueryResponse) Reset() {
	*x = QueryResponse{}
	mi := &file_google_cloud_bigquery_v2_job_proto_msgTypes[13]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *QueryResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*QueryResponse) ProtoMessage() {}

func (x *QueryResponse) ProtoReflect() protoreflect.Message {
	mi := &file_google_cloud_bigquery_v2_job_proto_msgTypes[13]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

func (x *QueryResponse) GetKind() string {
	if x != nil {
		return x.xxx_hidden_Kind
	}
	return ""
}

func (x *QueryResponse) GetSchema() *TableSchema {
	if x != nil {
		return x.xxx_hidden_Schema
	}
	return nil
}

func (x *QueryResponse) GetJobReference() *JobReference {
	if x != nil {
		return x.xxx_hidden_JobReference
	}
	return nil
}

func (x *QueryResponse) GetJobCreationReason() *JobCreationReason {
	if x != nil {
		return x.xxx_hidden_JobCreationReason
	}
	return nil
}

func (x *QueryResponse) GetQueryId() string {
	if x != nil {
		return x.xxx_hidden_QueryId
	}
	return ""
}

func (x *QueryResponse) GetLocation() string {
	if x != nil {
		return x.xxx_hidden_Location
	}
	return ""
}

func (x *QueryResponse) GetTotalRows() *wrapperspb.UInt64Value {
	if x != nil {
		return x.xxx_hidden_TotalRows
	}
	return nil
}

func (x *QueryResponse) GetPageToken() string {
	if x != nil {
		return x.xxx_hidden_PageToken
	}
	return ""
}

func (x *QueryResponse) GetRows() []*structpb.Struct {
	if x != nil {
		if x.xxx_hidden_Rows != nil {
			return *x.xxx_hidden_Rows
		}
	}
	return nil
}

func (x *QueryResponse) GetTotalBytesProcessed() *wrapperspb.Int64Value {
	if x != nil {
		return x.xxx_hidden_TotalBytesProcessed
	}
	return nil
}

func (x *QueryResponse) GetTotalBytesBilled() int64 {
	if x != nil {
		return x.xxx_hidden_TotalBytesBilled
	}
	return 0
}

func (x *QueryResponse) GetTotalSlotMs() int64 {
	if x != nil {
		return x.xxx_hidden_TotalSlotMs
	}
	return 0
}

func (x *QueryResponse) GetJobComplete() *wrapperspb.BoolValue {
	if x != nil {
		return x.xxx_hidden_JobComplete
	}
	return nil
}

func (x *QueryResponse) GetErrors() []*ErrorProto {
	if x != nil {
		if x.xxx_hidden_Errors != nil {
			return *x.xxx_hidden_Errors
		}
	}
	return nil
}

func (x *QueryResponse) GetCacheHit() *wrapperspb.BoolValue {
	if x != nil {
		return x.xxx_hidden_CacheHit
	}
	return nil
}

func (x *QueryResponse) GetNumDmlAffectedRows() *wrapperspb.Int64Value {
	if x != nil {
		return x.xxx_hidden_NumDmlAffectedRows
	}
	return nil
}

func (x *QueryResponse) GetSessionInfo() *SessionInfo {
	if x != nil {
		return x.xxx_hidden_SessionInfo
	}
	return nil
}

func (x *QueryResponse) GetDmlStats() *DmlStats {
	if x != nil {
		return x.xxx_hidden_DmlStats
	}
	return nil
}

func (x *QueryResponse) GetCreationTime() int64 {
	if x != nil {
		return x.xxx_hidden_CreationTime
	}
	return 0
}

func (x *QueryResponse) GetStartTime() int64 {
	if x != nil {
		return x.xxx_hidden_StartTime
	}
	return 0
}

func (x *QueryResponse) GetEndTime() int64 {
	if x != nil {
		return x.xxx_hidden_EndTime
	}
	return 0
}

func (x *QueryResponse) GetArrowSchema() *ArrowSchema {
	if x != nil {
		if x, ok := x.xxx_hidden_ResultsSchema.(*queryResponse_ArrowSchema); ok {
			return x.ArrowSchema
		}
	}
	return nil
}

func (x *QueryResponse) GetArrowRecordBatch() *ArrowRecordBatch {
	if x != nil {
		if x, ok := x.xxx_hidden_Results.(*queryResponse_ArrowRecordBatch); ok {
			return x.ArrowRecordBatch
		}
	}
	return nil
}

func (x *QueryResponse) GetPageRowCount() int64 {
	if x != nil {
		return x.xxx_hidden_PageRowCount
	}
	return 0
}

func (x *QueryResponse) GetStatementType() string {
	if x != nil {
		return x.xxx_hidden_StatementType
	}
	return ""
}

func (x *QueryResponse) SetKind(v string) {
	x.xxx_hidden_Kind = v
}

func (x *QueryResponse) SetSchema(v *TableSchema) {
	x.xxx_hidden_Schema = v
}

func (x *QueryResponse) SetJobReference(v *JobReference) {
	x.xxx_hidden_JobReference = v
}

func (x *QueryResponse) SetJobCreationReason(v *JobCreationReason) {
	x.xxx_hidden_JobCreationReason = v
}

func (x *QueryResponse) SetQueryId(v string) {
	x.xxx_hidden_QueryId = v
}

func (x *QueryResponse) SetLocation(v string) {
	x.xxx_hidden_Location = v
}

func (x *QueryResponse) SetTotalRows(v *wrapperspb.UInt64Value) {
	x.xxx_hidden_TotalRows = v
}

func (x *QueryResponse) SetPageToken(v string) {
	x.xxx_hidden_PageToken = v
}

func (x *QueryResponse) SetRows(v []*structpb.Struct) {
	x.xxx_hidden_Rows = &v
}

func (x *QueryResponse) SetTotalBytesProcessed(v *wrapperspb.Int64Value) {
	x.xxx_hidden_TotalBytesProcessed = v
}

func (x *QueryResponse) SetTotalBytesBilled(v int64) {
	x.xxx_hidden_TotalBytesBilled = v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 10, 25)
}

func (x *QueryResponse) SetTotalSlotMs(v int64) {
	x.xxx_hidden_TotalSlotMs = v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 11, 25)
}

func (x *QueryResponse) SetJobComplete(v *wrapperspb.BoolValue) {
	x.xxx_hidden_JobComplete = v
}

func (x *QueryResponse) SetErrors(v []*ErrorProto) {
	x.xxx_hidden_Errors = &v
}

func (x *QueryResponse) SetCacheHit(v *wrapperspb.BoolValue) {
	x.xxx_hidden_CacheHit = v
}

func (x *QueryResponse) SetNumDmlAffectedRows(v *wrapperspb.Int64Value) {
	x.xxx_hidden_NumDmlAffectedRows = v
}

func (x *QueryResponse) SetSessionInfo(v *SessionInfo) {
	x.xxx_hidden_SessionInfo = v
}

func (x *QueryResponse) SetDmlStats(v *DmlStats) {
	x.xxx_hidden_DmlStats = v
}

func (x *QueryResponse) SetCreationTime(v int64) {
	x.xxx_hidden_CreationTime = v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 18, 25)
}

func (x *QueryResponse) SetStartTime(v int64) {
	x.xxx_hidden_StartTime = v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 19, 25)
}

func (x *QueryResponse) SetEndTime(v int64) {
	x.xxx_hidden_EndTime = v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 20, 25)
}

func (x *QueryResponse) SetArrowSchema(v *ArrowSchema) {
	if v == nil {
		x.xxx_hidden_ResultsSchema = nil
		return
	}
	x.xxx_hidden_ResultsSchema = &queryResponse_ArrowSchema{v}
}

func (x *QueryResponse) SetArrowRecordBatch(v *ArrowRecordBatch) {
	if v == nil {
		x.xxx_hidden_Results = nil
		return
	}
	x.xxx_hidden_Results = &queryResponse_ArrowRecordBatch{v}
}

func (x *QueryResponse) SetPageRowCount(v int64) {
	x.xxx_hidden_PageRowCount = v
}

func (x *QueryResponse) SetStatementType(v string) {
	x.xxx_hidden_StatementType = v
}

func (x *QueryResponse) HasSchema() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_Schema != nil
}

func (x *QueryResponse) HasJobReference() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_JobReference != nil
}

func (x *QueryResponse) HasJobCreationReason() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_JobCreationReason != nil
}

func (x *QueryResponse) HasTotalRows() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_TotalRows != nil
}

func (x *QueryResponse) HasTotalBytesProcessed() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_TotalBytesProcessed != nil
}

func (x *QueryResponse) HasTotalBytesBilled() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 10)
}

func (x *QueryResponse) HasTotalSlotMs() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 11)
}

func (x *QueryResponse) HasJobComplete() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_JobComplete != nil
}

func (x *QueryResponse) HasCacheHit() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_CacheHit != nil
}

func (x *QueryResponse) HasNumDmlAffectedRows() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_NumDmlAffectedRows != nil
}

func (x *QueryResponse) HasSessionInfo() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_SessionInfo != nil
}

func (x *QueryResponse) HasDmlStats() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_DmlStats != nil
}

func (x *QueryResponse) HasCreationTime() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 18)
}

func (x *QueryResponse) HasStartTime() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 19)
}

func (x *QueryResponse) HasEndTime() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 20)
}

func (x *QueryResponse) HasResultsSchema() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_ResultsSchema != nil
}

func (x *QueryResponse) HasArrowSchema() bool {
	if x == nil {
		return false
	}
	_, ok := x.xxx_hidden_ResultsSchema.(*queryResponse_ArrowSchema)
	return ok
}

func (x *QueryResponse) HasResults() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_Results != nil
}

func (x *QueryResponse) HasArrowRecordBatch() bool {
	if x == nil {
		return false
	}
	_, ok := x.xxx_hidden_Results.(*queryResponse_ArrowRecordBatch)
	return ok
}

func (x *QueryResponse) ClearSchema() {
	x.xxx_hidden_Schema = nil
}

func (x *QueryResponse) ClearJobReference() {
	x.xxx_hidden_JobReference = nil
}

func (x *QueryResponse) ClearJobCreationReason() {
	x.xxx_hidden_JobCreationReason = nil
}

func (x *QueryResponse) ClearTotalRows() {
	x.xxx_hidden_TotalRows = nil
}

func (x *QueryResponse) ClearTotalBytesProcessed() {
	x.xxx_hidden_TotalBytesProcessed = nil
}

func (x *QueryResponse) ClearTotalBytesBilled() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 10)
	x.xxx_hidden_TotalBytesBilled = 0
}

func (x *QueryResponse) ClearTotalSlotMs() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 11)
	x.xxx_hidden_TotalSlotMs = 0
}

func (x *QueryResponse) ClearJobComplete() {
	x.xxx_hidden_JobComplete = nil
}

func (x *QueryResponse) ClearCacheHit() {
	x.xxx_hidden_CacheHit = nil
}

func (x *QueryResponse) ClearNumDmlAffectedRows() {
	x.xxx_hidden_NumDmlAffectedRows = nil
}

func (x *QueryResponse) ClearSessionInfo() {
	x.xxx_hidden_SessionInfo = nil
}

func (x *QueryResponse) ClearDmlStats() {
	x.xxx_hidden_DmlStats = nil
}

func (x *QueryResponse) ClearCreationTime() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 18)
	x.xxx_hidden_CreationTime = 0
}

func (x *QueryResponse) ClearStartTime() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 19)
	x.xxx_hidden_StartTime = 0
}

func (x *QueryResponse) ClearEndTime() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 20)
	x.xxx_hidden_EndTime = 0
}

func (x *QueryResponse) ClearResultsSchema() {
	x.xxx_hidden_ResultsSchema = nil
}

func (x *QueryResponse) ClearArrowSchema() {
	if _, ok := x.xxx_hidden_ResultsSchema.(*queryResponse_ArrowSchema); ok {
		x.xxx_hidden_ResultsSchema = nil
	}
}

func (x *QueryResponse) ClearResults() {
	x.xxx_hidden_Results = nil
}

func (x *QueryResponse) ClearArrowRecordBatch() {
	if _, ok := x.xxx_hidden_Results.(*queryResponse_ArrowRecordBatch); ok {
		x.xxx_hidden_Results = nil
	}
}

const QueryResponse_ResultsSchema_not_set_case case_QueryResponse_ResultsSchema = 0
const QueryResponse_ArrowSchema_case case_QueryResponse_ResultsSchema = 23

func (x *QueryResponse) WhichResultsSchema() case_QueryResponse_ResultsSchema {
	if x == nil {
		return QueryResponse_ResultsSchema_not_set_case
	}
	switch x.xxx_hidden_ResultsSchema.(type) {
	case *queryResponse_ArrowSchema:
		return QueryResponse_ArrowSchema_case
	default:
		return QueryResponse_ResultsSchema_not_set_case
	}
}

const QueryResponse_Results_not_set_case case_QueryResponse_Results = 0
const QueryResponse_ArrowRecordBatch_case case_QueryResponse_Results = 24

func (x *QueryResponse) WhichResults() case_QueryResponse_Results {
	if x == nil {
		return QueryResponse_Results_not_set_case
	}
	switch x.xxx_hidden_Results.(type) {
	case *queryResponse_ArrowRecordBatch:
		return QueryResponse_ArrowRecordBatch_case
	default:
		return QueryResponse_Results_not_set_case
	}
}

type QueryResponse_builder struct {
	_ [0]func() // Prevents comparability and use of unkeyed literals for the builder.

	// The resource type.
	Kind string
	// The schema of the results. Present only when the query completes
	// successfully.
	Schema *TableSchema
	// Reference to the Job that was created to run the query. This field will be
	// present even if the original request timed out, in which case
	// GetQueryResults can be used to read the results once the query has
	// completed. Since this API only returns the first page of results,
	// subsequent pages can be fetched via the same mechanism (GetQueryResults).
	//
	// If job_creation_mode was set to `JOB_CREATION_OPTIONAL` and the query
	// completes without creating a job, this field will be empty.
	JobReference *JobReference
	// Optional. The reason why a Job was created.
	//
	// Only relevant when a job_reference is present in the response.
	// If job_reference is not present it will always be unset.
	JobCreationReason *JobCreationReason
	// Auto-generated ID for the query.
	QueryId string
	// Output only. The geographic location of the query.
	//
	// For more information about BigQuery locations, see:
	// https://cloud.google.com/bigquery/docs/locations
	Location string
	// The total number of rows in the complete query result set, which can be
	// more than the number of rows in this single page of results.
	TotalRows *wrapperspb.UInt64Value
	// A token used for paging results. A non-empty token indicates that
	// additional results are available. To see additional results,
	// query the
	// [`jobs.getQueryResults`](https://cloud.google.com/bigquery/docs/reference/rest/v2/jobs/getQueryResults)
	// method. For more information, see [Paging through table
	// data](https://cloud.google.com/bigquery/docs/paging-results).
	PageToken string
	// An object with as many results as can be contained within the maximum
	// permitted reply size. To get any additional rows, you can call
	// GetQueryResults and specify the jobReference returned above.
	Rows []*structpb.Struct
	// The total number of bytes processed for this query. If this query was a dry
	// run, this is the number of bytes that would be processed if the query were
	// run.
	TotalBytesProcessed *wrapperspb.Int64Value
	// Output only. If the project is configured to use on-demand pricing,
	// then this field contains the total bytes billed for the job.
	// If the project is configured to use flat-rate pricing, then you are
	// not billed for bytes and this field is informational only.
	TotalBytesBilled *int64
	// Output only. Number of slot ms the user is actually billed for.
	TotalSlotMs *int64
	// Whether the query has completed or not. If rows or totalRows are present,
	// this will always be true. If this is false, totalRows will not be
	// available.
	JobComplete *wrapperspb.BoolValue
	// Output only. The first errors or warnings encountered during the running of
	// the job. The final message includes the number of errors that caused the
	// process to stop. Errors here do not necessarily mean that the job has
	// completed or was unsuccessful. For more information about error messages,
	// see [Error
	// messages](https://cloud.google.com/bigquery/docs/error-messages).
	Errors []*ErrorProto
	// Whether the query result was fetched from the query cache.
	CacheHit *wrapperspb.BoolValue
	// Output only. The number of rows affected by a DML statement. Present only
	// for DML statements INSERT, UPDATE or DELETE.
	NumDmlAffectedRows *wrapperspb.Int64Value
	// Output only. Information of the session if this job is part of one.
	SessionInfo *SessionInfo
	// Output only. Detailed statistics for DML statements INSERT, UPDATE, DELETE,
	// MERGE or TRUNCATE.
	DmlStats *DmlStats
	// Output only. Creation time of this query, in milliseconds since the epoch.
	// This field will be present on all queries.
	CreationTime *int64
	// Output only. Start time of this query, in milliseconds since the epoch.
	// This field will be present when the query job transitions from the PENDING
	// state to either RUNNING or DONE.
	StartTime *int64
	// Output only. End time of this query, in milliseconds since the epoch. This
	// field will be present whenever a query job is in the DONE state.
	EndTime *int64
	// The schema if `query_results_format` has been specified with value other
	// than default `STRUCT_ENCODING`.
	//
	// This feature is not yet available.

	// Fields of oneof xxx_hidden_ResultsSchema:
	// Output only. Arrow schema
	ArrowSchema *ArrowSchema
	// -- end of xxx_hidden_ResultsSchema
	// Row data is returned in format when `query_results_format` has been
	// specified with value other than default `STRUCT_ENCODING`.
	//
	// This feature is not yet available.

	// Fields of oneof xxx_hidden_Results:
	// Output only. Serialized row data in Arrow RecordBatch format.
	ArrowRecordBatch *ArrowRecordBatch
	// -- end of xxx_hidden_Results
	// Output only. The number of rows out of `total_rows` returned in this
	// response.
	//
	// This feature is not yet available.
	PageRowCount int64
	// Output only. The type of query statement, if valid.
	// Possible values:
	//
	// * `SELECT`:
	// [`SELECT`](https://cloud.google.com/bigquery/docs/reference/standard-sql/query-syntax#select_list)
	// statement.
	// * `ASSERT`:
	// [`ASSERT`](https://cloud.google.com/bigquery/docs/reference/standard-sql/debugging-statements#assert)
	// statement.
	// * `INSERT`:
	// [`INSERT`](https://cloud.google.com/bigquery/docs/reference/standard-sql/dml-syntax#insert_statement)
	// statement.
	// * `UPDATE`:
	// [`UPDATE`](https://cloud.google.com/bigquery/docs/reference/standard-sql/dml-syntax#update_statement)
	// statement.
	// * `DELETE`:
	// [`DELETE`](https://cloud.google.com/bigquery/docs/reference/standard-sql/data-manipulation-language)
	// statement.
	// * `MERGE`:
	// [`MERGE`](https://cloud.google.com/bigquery/docs/reference/standard-sql/data-manipulation-language)
	// statement.
	// * `CREATE_TABLE`: [`CREATE
	// TABLE`](https://cloud.google.com/bigquery/docs/reference/standard-sql/data-definition-language#create_table_statement)
	// statement, without `AS SELECT`.
	// * `CREATE_TABLE_AS_SELECT`: [`CREATE TABLE AS
	// SELECT`](https://cloud.google.com/bigquery/docs/reference/standard-sql/data-definition-language#create_table_statement)
	// statement.
	// * `CREATE_VIEW`: [`CREATE
	// VIEW`](https://cloud.google.com/bigquery/docs/reference/standard-sql/data-definition-language#create_view_statement)
	// statement.
	// * `CREATE_MODEL`: [`CREATE
	// MODEL`](https://cloud.google.com/bigquery-ml/docs/reference/standard-sql/bigqueryml-syntax-create#create_model_statement)
	// statement.
	// * `CREATE_MATERIALIZED_VIEW`: [`CREATE MATERIALIZED
	// VIEW`](https://cloud.google.com/bigquery/docs/reference/standard-sql/data-definition-language#create_materialized_view_statement)
	// statement.
	// * `CREATE_FUNCTION`: [`CREATE
	// FUNCTION`](https://cloud.google.com/bigquery/docs/reference/standard-sql/data-definition-language#create_function_statement)
	// statement.
	// * `CREATE_TABLE_FUNCTION`: [`CREATE TABLE
	// FUNCTION`](https://cloud.google.com/bigquery/docs/reference/standard-sql/data-definition-language#create_table_function_statement)
	// statement.
	// * `CREATE_PROCEDURE`: [`CREATE
	// PROCEDURE`](https://cloud.google.com/bigquery/docs/reference/standard-sql/data-definition-language#create_procedure)
	// statement.
	// * `CREATE_ROW_ACCESS_POLICY`: [`CREATE ROW ACCESS
	// POLICY`](https://cloud.google.com/bigquery/docs/reference/standard-sql/data-definition-language#create_row_access_policy_statement)
	// statement.
	// * `CREATE_SCHEMA`: [`CREATE
	// SCHEMA`](https://cloud.google.com/bigquery/docs/reference/standard-sql/data-definition-language#create_schema_statement)
	// statement.
	// * `CREATE_SNAPSHOT_TABLE`: [`CREATE SNAPSHOT
	// TABLE`](https://cloud.google.com/bigquery/docs/reference/standard-sql/data-definition-language#create_snapshot_table_statement)
	// statement.
	// * `CREATE_SEARCH_INDEX`: [`CREATE SEARCH
	// INDEX`](https://cloud.google.com/bigquery/docs/reference/standard-sql/data-definition-language#create_search_index_statement)
	// statement.
	// * `DROP_TABLE`: [`DROP
	// TABLE`](https://cloud.google.com/bigquery/docs/reference/standard-sql/data-definition-language#drop_table_statement)
	// statement.
	// * `DROP_EXTERNAL_TABLE`: [`DROP EXTERNAL
	// TABLE`](https://cloud.google.com/bigquery/docs/reference/standard-sql/data-definition-language#drop_external_table_statement)
	// statement.
	// * `DROP_VIEW`: [`DROP
	// VIEW`](https://cloud.google.com/bigquery/docs/reference/standard-sql/data-definition-language#drop_view_statement)
	// statement.
	// * `DROP_MODEL`: [`DROP
	// MODEL`](https://cloud.google.com/bigquery-ml/docs/reference/standard-sql/bigqueryml-syntax-drop-model)
	// statement.
	//   - `DROP_MATERIALIZED_VIEW`: [`DROP MATERIALIZED
	//     VIEW`](https://cloud.google.com/bigquery/docs/reference/standard-sql/data-definition-language#drop_materialized_view_statement)
	//
	// statement.
	// * `DROP_FUNCTION` : [`DROP
	// FUNCTION`](https://cloud.google.com/bigquery/docs/reference/standard-sql/data-definition-language#drop_function_statement)
	// statement.
	// * `DROP_TABLE_FUNCTION` : [`DROP TABLE
	// FUNCTION`](https://cloud.google.com/bigquery/docs/reference/standard-sql/data-definition-language#drop_table_function)
	// statement.
	// * `DROP_PROCEDURE`: [`DROP
	// PROCEDURE`](https://cloud.google.com/bigquery/docs/reference/standard-sql/data-definition-language#drop_procedure_statement)
	// statement.
	// * `DROP_SEARCH_INDEX`: [`DROP SEARCH
	// INDEX`](https://cloud.google.com/bigquery/docs/reference/standard-sql/data-definition-language#drop_search_index)
	// statement.
	// * `DROP_SCHEMA`: [`DROP
	// SCHEMA`](https://cloud.google.com/bigquery/docs/reference/standard-sql/data-definition-language#drop_schema_statement)
	// statement.
	// * `DROP_SNAPSHOT_TABLE`: [`DROP SNAPSHOT
	// TABLE`](https://cloud.google.com/bigquery/docs/reference/standard-sql/data-definition-language#drop_snapshot_table_statement)
	// statement.
	// * `DROP_ROW_ACCESS_POLICY`: [`DROP [ALL] ROW ACCESS
	// POLICY|POLICIES`](https://cloud.google.com/bigquery/docs/reference/standard-sql/data-definition-language#drop_row_access_policy_statement)
	// statement.
	// * `ALTER_TABLE`: [`ALTER
	// TABLE`](https://cloud.google.com/bigquery/docs/reference/standard-sql/data-definition-language#alter_table_set_options_statement)
	// statement.
	// * `ALTER_VIEW`: [`ALTER
	// VIEW`](https://cloud.google.com/bigquery/docs/reference/standard-sql/data-definition-language#alter_view_set_options_statement)
	// statement.
	// * `ALTER_MATERIALIZED_VIEW`: [`ALTER MATERIALIZED
	// VIEW`](https://cloud.google.com/bigquery/docs/reference/standard-sql/data-definition-language#alter_materialized_view_set_options_statement)
	// statement.
	// * `ALTER_SCHEMA`: [`ALTER
	// SCHEMA`](https://cloud.google.com/bigquery/docs/reference/standard-sql/data-definition-language#alter_schema_set_options_statement)
	// statement.
	// * `SCRIPT`:
	// [`SCRIPT`](https://cloud.google.com/bigquery/docs/reference/standard-sql/procedural-language).
	// * `TRUNCATE_TABLE`: [`TRUNCATE
	// TABLE`](https://cloud.google.com/bigquery/docs/reference/standard-sql/dml-syntax#truncate_table_statement)
	// statement.
	// * `CREATE_EXTERNAL_TABLE`: [`CREATE EXTERNAL
	// TABLE`](https://cloud.google.com/bigquery/docs/reference/standard-sql/data-definition-language#create_external_table_statement)
	// statement.
	// * `EXPORT_DATA`: [`EXPORT
	// DATA`](https://cloud.google.com/bigquery/docs/reference/standard-sql/other-statements#export_data_statement)
	// statement.
	// * `EXPORT_MODEL`: [`EXPORT
	// MODEL`](https://cloud.google.com/bigquery-ml/docs/reference/standard-sql/bigqueryml-syntax-export-model)
	// statement.
	// * `LOAD_DATA`: [`LOAD
	// DATA`](https://cloud.google.com/bigquery/docs/reference/standard-sql/other-statements#load_data_statement)
	// statement.
	// * `CALL`:
	// [`CALL`](https://cloud.google.com/bigquery/docs/reference/standard-sql/procedural-language#call)
	// statement.
	StatementType string
}

func (b0 QueryResponse_builder) Build() *QueryResponse {
	m0 := &QueryResponse{}
	b, x := &b0, m0
	_, _ = b, x
	x.xxx_hidden_Kind = b.Kind
	x.xxx_hidden_Schema = b.Schema
	x.xxx_hidden_JobReference = b.JobReference
	x.xxx_hidden_JobCreationReason = b.JobCreationReason
	x.xxx_hidden_QueryId = b.QueryId
	x.xxx_hidden_Location = b.Location
	x.xxx_hidden_TotalRows = b.TotalRows
	x.xxx_hidden_PageToken = b.PageToken
	x.xxx_hidden_Rows = &b.Rows
	x.xxx_hidden_TotalBytesProcessed = b.TotalBytesProcessed
	if b.TotalBytesBilled != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 10, 25)
		x.xxx_hidden_TotalBytesBilled = *b.TotalBytesBilled
	}
	if b.TotalSlotMs != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 11, 25)
		x.xxx_hidden_TotalSlotMs = *b.TotalSlotMs
	}
	x.xxx_hidden_JobComplete = b.JobComplete
	x.xxx_hidden_Errors = &b.Errors
	x.xxx_hidden_CacheHit = b.CacheHit
	x.xxx_hidden_NumDmlAffectedRows = b.NumDmlAffectedRows
	x.xxx_hidden_SessionInfo = b.SessionInfo
	x.xxx_hidden_DmlStats = b.DmlStats
	if b.CreationTime != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 18, 25)
		x.xxx_hidden_CreationTime = *b.CreationTime
	}
	if b.StartTime != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 19, 25)
		x.xxx_hidden_StartTime = *b.StartTime
	}
	if b.EndTime != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 20, 25)
		x.xxx_hidden_EndTime = *b.EndTime
	}
	if b.ArrowSchema != nil {
		x.xxx_hidden_ResultsSchema = &queryResponse_ArrowSchema{b.ArrowSchema}
	}
	if b.ArrowRecordBatch != nil {
		x.xxx_hidden_Results = &queryResponse_ArrowRecordBatch{b.ArrowRecordBatch}
	}
	x.xxx_hidden_PageRowCount = b.PageRowCount
	x.xxx_hidden_StatementType = b.StatementType
	return m0
}

type case_QueryResponse_ResultsSchema protoreflect.FieldNumber

func (x case_QueryResponse_ResultsSchema) String() string {
	md := file_google_cloud_bigquery_v2_job_proto_msgTypes[13].Descriptor()
	if x == 0 {
		return "not set"
	}
	return protoimpl.X.MessageFieldStringOf(md, protoreflect.FieldNumber(x))
}

type case_QueryResponse_Results protoreflect.FieldNumber

func (x case_QueryResponse_Results) String() string {
	md := file_google_cloud_bigquery_v2_job_proto_msgTypes[13].Descriptor()
	if x == 0 {
		return "not set"
	}
	return protoimpl.X.MessageFieldStringOf(md, protoreflect.FieldNumber(x))
}

type isQueryResponse_ResultsSchema interface {
	isQueryResponse_ResultsSchema()
}

type queryResponse_ArrowSchema struct {
	// Output only. Arrow schema
	ArrowSchema *ArrowSchema `protobuf:"bytes,23,opt,name=arrow_schema,json=arrowSchema,proto3,oneof"`
}

func (*queryResponse_ArrowSchema) isQueryResponse_ResultsSchema() {}

type isQueryResponse_Results interface {
	isQueryResponse_Results()
}

type queryResponse_ArrowRecordBatch struct {
	// Output only. Serialized row data in Arrow RecordBatch format.
	ArrowRecordBatch *ArrowRecordBatch `protobuf:"bytes,24,opt,name=arrow_record_batch,json=arrowRecordBatch,proto3,oneof"`
}

func (*queryResponse_ArrowRecordBatch) isQueryResponse_Results() {}

var File_google_cloud_bigquery_v2_job_proto protoreflect.FileDescriptor

const file_google_cloud_bigquery_v2_job_proto_rawDesc = "" +
	"\n" +
	"\"google/cloud/bigquery/v2/job.proto\x12\x18google.cloud.bigquery.v2\x1a\x1cgoogle/api/annotations.proto\x1a\x17google/api/client.proto\x1a\x1fgoogle/api/field_behavior.proto\x1a\x19google/api/resource.proto\x1a$google/cloud/bigquery/v2/arrow.proto\x1a2google/cloud/bigquery/v2/data_format_options.proto\x1a0google/cloud/bigquery/v2/dataset_reference.proto\x1a0google/cloud/bigquery/v2/encryption_config.proto\x1a$google/cloud/bigquery/v2/error.proto\x1a)google/cloud/bigquery/v2/job_config.proto\x1a2google/cloud/bigquery/v2/job_creation_reason.proto\x1a,google/cloud/bigquery/v2/job_reference.proto\x1a(google/cloud/bigquery/v2/job_stats.proto\x1a)google/cloud/bigquery/v2/job_status.proto\x1a.google/cloud/bigquery/v2/query_parameter.proto\x1a+google/cloud/bigquery/v2/session_info.proto\x1a+google/cloud/bigquery/v2/table_schema.proto\x1a\x1bgoogle/protobuf/empty.proto\x1a\x1cgoogle/protobuf/struct.proto\x1a\x1egoogle/protobuf/wrappers.proto\"\xe1\x04\n" +
	"\x03Job\x12\x17\n" +
	"\x04kind\x18\x01 \x01(\tB\x03\xe0A\x03R\x04kind\x12\x17\n" +
	"\x04etag\x18\x02 \x01(\tB\x03\xe0A\x03R\x04etag\x12\x13\n" +
	"\x02id\x18\x03 \x01(\tB\x03\xe0A\x03R\x02id\x12 \n" +
	"\tself_link\x18\x04 \x01(\tB\x03\xe0A\x03R\bselfLink\x12#\n" +
	"\n" +
	"user_email\x18\x05 \x01(\tB\x03\xe0A\x03R\n" +
	"user_email\x12U\n" +
	"\rconfiguration\x18\x06 \x01(\v2*.google.cloud.bigquery.v2.JobConfigurationB\x03\xe0A\x02R\rconfiguration\x12P\n" +
	"\rjob_reference\x18\a \x01(\v2&.google.cloud.bigquery.v2.JobReferenceB\x03\xe0A\x01R\fjobReference\x12L\n" +
	"\n" +
	"statistics\x18\b \x01(\v2'.google.cloud.bigquery.v2.JobStatisticsB\x03\xe0A\x03R\n" +
	"statistics\x12@\n" +
	"\x06status\x18\t \x01(\v2#.google.cloud.bigquery.v2.JobStatusB\x03\xe0A\x03R\x06status\x121\n" +
	"\x11principal_subject\x18\r \x01(\tB\x03\xe0A\x03R\x11principal_subject\x12`\n" +
	"\x13job_creation_reason\x18\x0e \x01(\v2+.google.cloud.bigquery.v2.JobCreationReasonB\x03\xe0A\x03R\x11jobCreationReason\"n\n" +
	"\x10CancelJobRequest\x12\"\n" +
	"\n" +
	"project_id\x18\x01 \x01(\tB\x03\xe0A\x02R\tprojectId\x12\x1a\n" +
	"\x06job_id\x18\x02 \x01(\tB\x03\xe0A\x02R\x05jobId\x12\x1a\n" +
	"\blocation\x18\x03 \x01(\tR\blocation\"X\n" +
	"\x11JobCancelResponse\x12\x12\n" +
	"\x04kind\x18\x01 \x01(\tR\x04kind\x12/\n" +
	"\x03job\x18\x02 \x01(\v2\x1d.google.cloud.bigquery.v2.JobR\x03job\"k\n" +
	"\rGetJobRequest\x12\"\n" +
	"\n" +
	"project_id\x18\x01 \x01(\tB\x03\xe0A\x02R\tprojectId\x12\x1a\n" +
	"\x06job_id\x18\x02 \x01(\tB\x03\xe0A\x02R\x05jobId\x12\x1a\n" +
	"\blocation\x18\x03 \x01(\tR\blocation\"b\n" +
	"\x10InsertJobRequest\x12\x1d\n" +
	"\n" +
	"project_id\x18\x01 \x01(\tR\tprojectId\x12/\n" +
	"\x03job\x18\x03 \x01(\v2\x1d.google.cloud.bigquery.v2.JobR\x03job\"n\n" +
	"\x10DeleteJobRequest\x12\"\n" +
	"\n" +
	"project_id\x18\x01 \x01(\tB\x03\xe0A\x02R\tprojectId\x12\x1a\n" +
	"\x06job_id\x18\x02 \x01(\tB\x03\xe0A\x02R\x05jobId\x12\x1a\n" +
	"\blocation\x18\x03 \x01(\tR\blocation\"\x8f\x05\n" +
	"\x0fListJobsRequest\x12\x1d\n" +
	"\n" +
	"project_id\x18\x01 \x01(\tR\tprojectId\x12\x1b\n" +
	"\tall_users\x18\x02 \x01(\bR\ballUsers\x12<\n" +
	"\vmax_results\x18\x03 \x01(\v2\x1b.google.protobuf.Int32ValueR\n" +
	"maxResults\x12*\n" +
	"\x11min_creation_time\x18\x04 \x01(\x04R\x0fminCreationTime\x12H\n" +
	"\x11max_creation_time\x18\x05 \x01(\v2\x1c.google.protobuf.UInt64ValueR\x0fmaxCreationTime\x12\x1d\n" +
	"\n" +
	"page_token\x18\x06 \x01(\tR\tpageToken\x12T\n" +
	"\n" +
	"projection\x18\a \x01(\x0e24.google.cloud.bigquery.v2.ListJobsRequest.ProjectionR\n" +
	"projection\x12X\n" +
	"\fstate_filter\x18\b \x03(\x0e25.google.cloud.bigquery.v2.ListJobsRequest.StateFilterR\vstateFilter\x12\"\n" +
	"\rparent_job_id\x18\t \x01(\tR\vparentJobId\">\n" +
	"\n" +
	"Projection\x12\v\n" +
	"\aminimal\x10\x00\x12\v\n" +
	"\aMINIMAL\x10\x00\x12\b\n" +
	"\x04full\x10\x01\x12\b\n" +
	"\x04FULL\x10\x01\x1a\x02\x10\x01\"Y\n" +
	"\vStateFilter\x12\b\n" +
	"\x04done\x10\x00\x12\b\n" +
	"\x04DONE\x10\x00\x12\v\n" +
	"\apending\x10\x01\x12\v\n" +
	"\aPENDING\x10\x01\x12\v\n" +
	"\arunning\x10\x02\x12\v\n" +
	"\aRUNNING\x10\x02\x1a\x02\x10\x01\"\x8f\x04\n" +
	"\rListFormatJob\x12\x0e\n" +
	"\x02id\x18\x01 \x01(\tR\x02id\x12\x12\n" +
	"\x04kind\x18\x02 \x01(\tR\x04kind\x12K\n" +
	"\rjob_reference\x18\x03 \x01(\v2&.google.cloud.bigquery.v2.JobReferenceR\fjobReference\x12\x14\n" +
	"\x05state\x18\x04 \x01(\tR\x05state\x12G\n" +
	"\ferror_result\x18\x05 \x01(\v2$.google.cloud.bigquery.v2.ErrorProtoR\verrorResult\x12L\n" +
	"\n" +
	"statistics\x18\x06 \x01(\v2'.google.cloud.bigquery.v2.JobStatisticsB\x03\xe0A\x03R\n" +
	"statistics\x12U\n" +
	"\rconfiguration\x18\a \x01(\v2*.google.cloud.bigquery.v2.JobConfigurationB\x03\xe0A\x02R\rconfiguration\x12;\n" +
	"\x06status\x18\b \x01(\v2#.google.cloud.bigquery.v2.JobStatusR\x06status\x12\x1e\n" +
	"\n" +
	"user_email\x18\t \x01(\tR\n" +
	"user_email\x12,\n" +
	"\x11principal_subject\x18\n" +
	" \x01(\tR\x11principal_subject\"\xb8\x01\n" +
	"\aJobList\x12\x12\n" +
	"\x04etag\x18\x01 \x01(\tR\x04etag\x12\x12\n" +
	"\x04kind\x18\x02 \x01(\tR\x04kind\x12&\n" +
	"\x0fnext_page_token\x18\x03 \x01(\tR\rnextPageToken\x12;\n" +
	"\x04jobs\x18\x04 \x03(\v2'.google.cloud.bigquery.v2.ListFormatJobR\x04jobs\x12 \n" +
	"\vunreachable\x18\x05 \x03(\tR\vunreachable\"\xa7\x03\n" +
	"\x16GetQueryResultsRequest\x12\"\n" +
	"\n" +
	"project_id\x18\x01 \x01(\tB\x03\xe0A\x02R\tprojectId\x12\x1a\n" +
	"\x06job_id\x18\x02 \x01(\tB\x03\xe0A\x02R\x05jobId\x12=\n" +
	"\vstart_index\x18\x03 \x01(\v2\x1c.google.protobuf.UInt64ValueR\n" +
	"startIndex\x12\x1d\n" +
	"\n" +
	"page_token\x18\x04 \x01(\tR\tpageToken\x12=\n" +
	"\vmax_results\x18\x05 \x01(\v2\x1c.google.protobuf.UInt32ValueR\n" +
	"maxResults\x12;\n" +
	"\n" +
	"timeout_ms\x18\x06 \x01(\v2\x1c.google.protobuf.UInt32ValueR\ttimeoutMs\x12\x1a\n" +
	"\blocation\x18\a \x01(\tR\blocation\x12W\n" +
	"\x0eformat_options\x18\b \x01(\v2+.google.cloud.bigquery.v2.DataFormatOptionsB\x03\xe0A\x01R\rformatOptions\"\xb7\x05\n" +
	"\x17GetQueryResultsResponse\x12\x12\n" +
	"\x04kind\x18\x01 \x01(\tR\x04kind\x12\x12\n" +
	"\x04etag\x18\x02 \x01(\tR\x04etag\x12=\n" +
	"\x06schema\x18\x03 \x01(\v2%.google.cloud.bigquery.v2.TableSchemaR\x06schema\x12K\n" +
	"\rjob_reference\x18\x04 \x01(\v2&.google.cloud.bigquery.v2.JobReferenceR\fjobReference\x12;\n" +
	"\n" +
	"total_rows\x18\x05 \x01(\v2\x1c.google.protobuf.UInt64ValueR\ttotalRows\x12\x1d\n" +
	"\n" +
	"page_token\x18\x06 \x01(\tR\tpageToken\x12+\n" +
	"\x04rows\x18\a \x03(\v2\x17.google.protobuf.StructR\x04rows\x12O\n" +
	"\x15total_bytes_processed\x18\b \x01(\v2\x1b.google.protobuf.Int64ValueR\x13totalBytesProcessed\x12=\n" +
	"\fjob_complete\x18\t \x01(\v2\x1a.google.protobuf.BoolValueR\vjobComplete\x12A\n" +
	"\x06errors\x18\n" +
	" \x03(\v2$.google.cloud.bigquery.v2.ErrorProtoB\x03\xe0A\x03R\x06errors\x127\n" +
	"\tcache_hit\x18\v \x01(\v2\x1a.google.protobuf.BoolValueR\bcacheHit\x12S\n" +
	"\x15num_dml_affected_rows\x18\f \x01(\v2\x1b.google.protobuf.Int64ValueB\x03\xe0A\x03R\x12numDmlAffectedRows\"\x83\x01\n" +
	"\x10PostQueryRequest\x12\"\n" +
	"\n" +
	"project_id\x18\x01 \x01(\tB\x03\xe0A\x02R\tprojectId\x12K\n" +
	"\rquery_request\x18\x02 \x01(\v2&.google.cloud.bigquery.v2.QueryRequestR\fqueryRequest\"\xe5\x10\n" +
	"\fQueryRequest\x12\x12\n" +
	"\x04kind\x18\x02 \x01(\tR\x04kind\x12\x19\n" +
	"\x05query\x18\x03 \x01(\tB\x03\xe0A\x02R\x05query\x12B\n" +
	"\vmax_results\x18\x04 \x01(\v2\x1c.google.protobuf.UInt32ValueB\x03\xe0A\x01R\n" +
	"maxResults\x12X\n" +
	"\x0fdefault_dataset\x18\x05 \x01(\v2*.google.cloud.bigquery.v2.DatasetReferenceB\x03\xe0A\x01R\x0edefaultDataset\x12@\n" +
	"\n" +
	"timeout_ms\x18\x06 \x01(\v2\x1c.google.protobuf.UInt32ValueB\x03\xe0A\x01R\ttimeoutMs\x12.\n" +
	"\x0ejob_timeout_ms\x18\x1a \x01(\x03B\x03\xe0A\x01H\x01R\fjobTimeoutMs\x88\x01\x01\x12%\n" +
	"\tmax_slots\x18\x1c \x01(\x05B\x03\xe0A\x01H\x02R\bmaxSlots\x88\x01\x01\x12\x88\x01\n" +
	"$destination_encryption_configuration\x18\x1b \x01(\v21.google.cloud.bigquery.v2.EncryptionConfigurationB\x03\xe0A\x01R\"destinationEncryptionConfiguration\x12\x1c\n" +
	"\adry_run\x18\a \x01(\bB\x03\xe0A\x01R\x06dryRun\x12G\n" +
	"\x0fuse_query_cache\x18\t \x01(\v2\x1a.google.protobuf.BoolValueB\x03\xe0A\x01R\ruseQueryCache\x12@\n" +
	"\x0euse_legacy_sql\x18\n" +
	" \x01(\v2\x1a.google.protobuf.BoolValueR\fuseLegacySql\x12%\n" +
	"\x0eparameter_mode\x18\v \x01(\tR\rparameterMode\x12S\n" +
	"\x10query_parameters\x18\f \x03(\v2(.google.cloud.bigquery.v2.QueryParameterR\x0fqueryParameters\x12\x1a\n" +
	"\blocation\x18\r \x01(\tR\blocation\x12W\n" +
	"\x0eformat_options\x18\x0f \x01(\v2+.google.cloud.bigquery.v2.DataFormatOptionsB\x03\xe0A\x01R\rformatOptions\x12f\n" +
	"\x15connection_properties\x18\x10 \x03(\v2,.google.cloud.bigquery.v2.ConnectionPropertyB\x03\xe0A\x01R\x14connectionProperties\x12O\n" +
	"\x06labels\x18\x11 \x03(\v22.google.cloud.bigquery.v2.QueryRequest.LabelsEntryB\x03\xe0A\x01R\x06labels\x12R\n" +
	"\x14maximum_bytes_billed\x18\x12 \x01(\v2\x1b.google.protobuf.Int64ValueB\x03\xe0A\x01R\x12maximumBytesBilled\x12\"\n" +
	"\n" +
	"request_id\x18\x13 \x01(\tB\x03\xe0A\x01R\trequestId\x12F\n" +
	"\x0ecreate_session\x18\x14 \x01(\v2\x1a.google.protobuf.BoolValueB\x03\xe0A\x01R\rcreateSession\x12g\n" +
	"\x11job_creation_mode\x18\x16 \x01(\x0e26.google.cloud.bigquery.v2.QueryRequest.JobCreationModeB\x03\xe0A\x01R\x0fjobCreationMode\x12]\n" +
	"\vreservation\x18\x18 \x01(\tB6\xe0A\x01\xfaA0\n" +
	".bigqueryreservation.googleapis.com/ReservationH\x03R\vreservation\x88\x01\x01\x12?\n" +
	"\x19write_incremental_results\x18\x19 \x01(\bB\x03\xe0A\x01R\x17writeIncrementalResults\x12p\n" +
	"\x14query_results_format\x18\x1d \x01(\x0e29.google.cloud.bigquery.v2.QueryRequest.QueryResultsFormatB\x03\xe0A\x01R\x12queryResultsFormat\x12z\n" +
	"\x1barrow_serialization_options\x18\x1e \x01(\v23.google.cloud.bigquery.v2.ArrowSerializationOptionsB\x03\xe0A\x01H\x00R\x19arrowSerializationOptions\x1a9\n" +
	"\vLabelsEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12\x14\n" +
	"\x05value\x18\x02 \x01(\tR\x05value:\x028\x01\"j\n" +
	"\x0fJobCreationMode\x12!\n" +
	"\x1dJOB_CREATION_MODE_UNSPECIFIED\x10\x00\x12\x19\n" +
	"\x15JOB_CREATION_REQUIRED\x10\x01\x12\x19\n" +
	"\x15JOB_CREATION_OPTIONAL\x10\x02\"Z\n" +
	"\x12QueryResultsFormat\x12$\n" +
	" QUERY_RESULTS_FORMAT_UNSPECIFIED\x10\x00\x12\x13\n" +
	"\x0fSTRUCT_ENCODING\x10\x01\x12\t\n" +
	"\x05ARROW\x10\x02B&\n" +
	"$results_format_serialization_optionsB\x11\n" +
	"\x0f_job_timeout_msB\f\n" +
	"\n" +
	"_max_slotsB\x0e\n" +
	"\f_reservation\"\xac\f\n" +
	"\rQueryResponse\x12\x12\n" +
	"\x04kind\x18\x01 \x01(\tR\x04kind\x12=\n" +
	"\x06schema\x18\x02 \x01(\v2%.google.cloud.bigquery.v2.TableSchemaR\x06schema\x12K\n" +
	"\rjob_reference\x18\x03 \x01(\v2&.google.cloud.bigquery.v2.JobReferenceR\fjobReference\x12`\n" +
	"\x13job_creation_reason\x18\x0f \x01(\v2+.google.cloud.bigquery.v2.JobCreationReasonB\x03\xe0A\x01R\x11jobCreationReason\x12\x19\n" +
	"\bquery_id\x18\x0e \x01(\tR\aqueryId\x12\x1f\n" +
	"\blocation\x18\x12 \x01(\tB\x03\xe0A\x03R\blocation\x12;\n" +
	"\n" +
	"total_rows\x18\x04 \x01(\v2\x1c.google.protobuf.UInt64ValueR\ttotalRows\x12\x1d\n" +
	"\n" +
	"page_token\x18\x05 \x01(\tR\tpageToken\x12+\n" +
	"\x04rows\x18\x06 \x03(\v2\x17.google.protobuf.StructR\x04rows\x12O\n" +
	"\x15total_bytes_processed\x18\a \x01(\v2\x1b.google.protobuf.Int64ValueR\x13totalBytesProcessed\x126\n" +
	"\x12total_bytes_billed\x18\x10 \x01(\x03B\x03\xe0A\x03H\x02R\x10totalBytesBilled\x88\x01\x01\x12,\n" +
	"\rtotal_slot_ms\x18\x11 \x01(\x03B\x03\xe0A\x03H\x03R\vtotalSlotMs\x88\x01\x01\x12=\n" +
	"\fjob_complete\x18\b \x01(\v2\x1a.google.protobuf.BoolValueR\vjobComplete\x12A\n" +
	"\x06errors\x18\t \x03(\v2$.google.cloud.bigquery.v2.ErrorProtoB\x03\xe0A\x03R\x06errors\x127\n" +
	"\tcache_hit\x18\n" +
	" \x01(\v2\x1a.google.protobuf.BoolValueR\bcacheHit\x12S\n" +
	"\x15num_dml_affected_rows\x18\v \x01(\v2\x1b.google.protobuf.Int64ValueB\x03\xe0A\x03R\x12numDmlAffectedRows\x12M\n" +
	"\fsession_info\x18\f \x01(\v2%.google.cloud.bigquery.v2.SessionInfoB\x03\xe0A\x03R\vsessionInfo\x12D\n" +
	"\tdml_stats\x18\r \x01(\v2\".google.cloud.bigquery.v2.DmlStatsB\x03\xe0A\x03R\bdmlStats\x12-\n" +
	"\rcreation_time\x18\x13 \x01(\x03B\x03\xe0A\x03H\x04R\fcreationTime\x88\x01\x01\x12'\n" +
	"\n" +
	"start_time\x18\x14 \x01(\x03B\x03\xe0A\x03H\x05R\tstartTime\x88\x01\x01\x12#\n" +
	"\bend_time\x18\x15 \x01(\x03B\x03\xe0A\x03H\x06R\aendTime\x88\x01\x01\x12O\n" +
	"\farrow_schema\x18\x17 \x01(\v2%.google.cloud.bigquery.v2.ArrowSchemaB\x03\xe0A\x03H\x00R\varrowSchema\x12_\n" +
	"\x12arrow_record_batch\x18\x18 \x01(\v2*.google.cloud.bigquery.v2.ArrowRecordBatchB\x03\xe0A\x03H\x01R\x10arrowRecordBatch\x12)\n" +
	"\x0epage_row_count\x18\x19 \x01(\x03B\x03\xe0A\x03R\fpageRowCount\x12*\n" +
	"\x0estatement_type\x18\x1a \x01(\tB\x03\xe0A\x03R\rstatementTypeB\x10\n" +
	"\x0eresults_schemaB\t\n" +
	"\aresultsB\x15\n" +
	"\x13_total_bytes_billedB\x10\n" +
	"\x0e_total_slot_msB\x10\n" +
	"\x0e_creation_timeB\r\n" +
	"\v_start_timeB\v\n" +
	"\t_end_time2\xb1\v\n" +
	"\n" +
	"JobService\x12\xa9\x01\n" +
	"\tCancelJob\x12*.google.cloud.bigquery.v2.CancelJobRequest\x1a+.google.cloud.bigquery.v2.JobCancelResponse\"C\x82\xd3\xe4\x93\x02=\";/bigquery/v2/projects/{project_id=*}/jobs/{job_id=*}/cancel\x12\x8e\x01\n" +
	"\x06GetJob\x12'.google.cloud.bigquery.v2.GetJobRequest\x1a\x1d.google.cloud.bigquery.v2.Job\"<\x82\xd3\xe4\x93\x026\x124/bigquery/v2/projects/{project_id=*}/jobs/{job_id=*}\x12\x8e\x01\n" +
	"\tInsertJob\x12*.google.cloud.bigquery.v2.InsertJobRequest\x1a\x1d.google.cloud.bigquery.v2.Job\"6\x82\xd3\xe4\x93\x020:\x03job\")/bigquery/v2/projects/{project_id=*}/jobs\x12\x94\x01\n" +
	"\tDeleteJob\x12*.google.cloud.bigquery.v2.DeleteJobRequest\x1a\x16.google.protobuf.Empty\"C\x82\xd3\xe4\x93\x02=*;/bigquery/v2/projects/{project_id=*}/jobs/{job_id=*}/delete\x12\x8b\x01\n" +
	"\bListJobs\x12).google.cloud.bigquery.v2.ListJobsRequest\x1a!.google.cloud.bigquery.v2.JobList\"1\x82\xd3\xe4\x93\x02+\x12)/bigquery/v2/projects/{project_id=*}/jobs\x12\xb7\x01\n" +
	"\x0fGetQueryResults\x120.google.cloud.bigquery.v2.GetQueryResultsRequest\x1a1.google.cloud.bigquery.v2.GetQueryResultsResponse\"?\x82\xd3\xe4\x93\x029\x127/bigquery/v2/projects/{project_id=*}/queries/{job_id=*}\x12\xa1\x01\n" +
	"\x05Query\x12*.google.cloud.bigquery.v2.PostQueryRequest\x1a'.google.cloud.bigquery.v2.QueryResponse\"C\x82\xd3\xe4\x93\x02=:\rquery_request\",/bigquery/v2/projects/{project_id=*}/queries\x1a\xd1\x02\xcaA\x17bigquery.googleapis.com\xd2A\xb3\x02https://www.googleapis.com/auth/bigquery,https://www.googleapis.com/auth/cloud-platform,https://www.googleapis.com/auth/cloud-platform.read-only,https://www.googleapis.com/auth/devstorage.full_control,https://www.googleapis.com/auth/devstorage.read_only,https://www.googleapis.com/auth/devstorage.read_writeBe\n" +
	"\x1ccom.google.cloud.bigquery.v2B\bJobProtoZ;cloud.google.com/go/bigquery/v2/apiv2/bigquerypb;bigquerypbb\x06proto3"

var file_google_cloud_bigquery_v2_job_proto_enumTypes = make([]protoimpl.EnumInfo, 4)
var file_google_cloud_bigquery_v2_job_proto_msgTypes = make([]protoimpl.MessageInfo, 15)
var file_google_cloud_bigquery_v2_job_proto_goTypes = []any{
	(ListJobsRequest_Projection)(0),      // 0: google.cloud.bigquery.v2.ListJobsRequest.Projection
	(ListJobsRequest_StateFilter)(0),     // 1: google.cloud.bigquery.v2.ListJobsRequest.StateFilter
	(QueryRequest_JobCreationMode)(0),    // 2: google.cloud.bigquery.v2.QueryRequest.JobCreationMode
	(QueryRequest_QueryResultsFormat)(0), // 3: google.cloud.bigquery.v2.QueryRequest.QueryResultsFormat
	(*Job)(nil),                          // 4: google.cloud.bigquery.v2.Job
	(*CancelJobRequest)(nil),             // 5: google.cloud.bigquery.v2.CancelJobRequest
	(*JobCancelResponse)(nil),            // 6: google.cloud.bigquery.v2.JobCancelResponse
	(*GetJobRequest)(nil),                // 7: google.cloud.bigquery.v2.GetJobRequest
	(*InsertJobRequest)(nil),             // 8: google.cloud.bigquery.v2.InsertJobRequest
	(*DeleteJobRequest)(nil),             // 9: google.cloud.bigquery.v2.DeleteJobRequest
	(*ListJobsRequest)(nil),              // 10: google.cloud.bigquery.v2.ListJobsRequest
	(*ListFormatJob)(nil),                // 11: google.cloud.bigquery.v2.ListFormatJob
	(*JobList)(nil),                      // 12: google.cloud.bigquery.v2.JobList
	(*GetQueryResultsRequest)(nil),       // 13: google.cloud.bigquery.v2.GetQueryResultsRequest
	(*GetQueryResultsResponse)(nil),      // 14: google.cloud.bigquery.v2.GetQueryResultsResponse
	(*PostQueryRequest)(nil),             // 15: google.cloud.bigquery.v2.PostQueryRequest
	(*QueryRequest)(nil),                 // 16: google.cloud.bigquery.v2.QueryRequest
	(*QueryResponse)(nil),                // 17: google.cloud.bigquery.v2.QueryResponse
	nil,                                  // 18: google.cloud.bigquery.v2.QueryRequest.LabelsEntry
	(*JobConfiguration)(nil),             // 19: google.cloud.bigquery.v2.JobConfiguration
	(*JobReference)(nil),                 // 20: google.cloud.bigquery.v2.JobReference
	(*JobStatistics)(nil),                // 21: google.cloud.bigquery.v2.JobStatistics
	(*JobStatus)(nil),                    // 22: google.cloud.bigquery.v2.JobStatus
	(*JobCreationReason)(nil),            // 23: google.cloud.bigquery.v2.JobCreationReason
	(*wrapperspb.Int32Value)(nil),        // 24: google.protobuf.Int32Value
	(*wrapperspb.UInt64Value)(nil),       // 25: google.protobuf.UInt64Value
	(*ErrorProto)(nil),                   // 26: google.cloud.bigquery.v2.ErrorProto
	(*wrapperspb.UInt32Value)(nil),       // 27: google.protobuf.UInt32Value
	(*DataFormatOptions)(nil),            // 28: google.cloud.bigquery.v2.DataFormatOptions
	(*TableSchema)(nil),                  // 29: google.cloud.bigquery.v2.TableSchema
	(*structpb.Struct)(nil),              // 30: google.protobuf.Struct
	(*wrapperspb.Int64Value)(nil),        // 31: google.protobuf.Int64Value
	(*wrapperspb.BoolValue)(nil),         // 32: google.protobuf.BoolValue
	(*DatasetReference)(nil),             // 33: google.cloud.bigquery.v2.DatasetReference
	(*EncryptionConfiguration)(nil),      // 34: google.cloud.bigquery.v2.EncryptionConfiguration
	(*QueryParameter)(nil),               // 35: google.cloud.bigquery.v2.QueryParameter
	(*ConnectionProperty)(nil),           // 36: google.cloud.bigquery.v2.ConnectionProperty
	(*ArrowSerializationOptions)(nil),    // 37: google.cloud.bigquery.v2.ArrowSerializationOptions
	(*SessionInfo)(nil),                  // 38: google.cloud.bigquery.v2.SessionInfo
	(*DmlStats)(nil),                     // 39: google.cloud.bigquery.v2.DmlStats
	(*ArrowSchema)(nil),                  // 40: google.cloud.bigquery.v2.ArrowSchema
	(*ArrowRecordBatch)(nil),             // 41: google.cloud.bigquery.v2.ArrowRecordBatch
	(*emptypb.Empty)(nil),                // 42: google.protobuf.Empty
}
var file_google_cloud_bigquery_v2_job_proto_depIdxs = []int32{
	19, // 0: google.cloud.bigquery.v2.Job.configuration:type_name -> google.cloud.bigquery.v2.JobConfiguration
	20, // 1: google.cloud.bigquery.v2.Job.job_reference:type_name -> google.cloud.bigquery.v2.JobReference
	21, // 2: google.cloud.bigquery.v2.Job.statistics:type_name -> google.cloud.bigquery.v2.JobStatistics
	22, // 3: google.cloud.bigquery.v2.Job.status:type_name -> google.cloud.bigquery.v2.JobStatus
	23, // 4: google.cloud.bigquery.v2.Job.job_creation_reason:type_name -> google.cloud.bigquery.v2.JobCreationReason
	4,  // 5: google.cloud.bigquery.v2.JobCancelResponse.job:type_name -> google.cloud.bigquery.v2.Job
	4,  // 6: google.cloud.bigquery.v2.InsertJobRequest.job:type_name -> google.cloud.bigquery.v2.Job
	24, // 7: google.cloud.bigquery.v2.ListJobsRequest.max_results:type_name -> google.protobuf.Int32Value
	25, // 8: google.cloud.bigquery.v2.ListJobsRequest.max_creation_time:type_name -> google.protobuf.UInt64Value
	0,  // 9: google.cloud.bigquery.v2.ListJobsRequest.projection:type_name -> google.cloud.bigquery.v2.ListJobsRequest.Projection
	1,  // 10: google.cloud.bigquery.v2.ListJobsRequest.state_filter:type_name -> google.cloud.bigquery.v2.ListJobsRequest.StateFilter
	20, // 11: google.cloud.bigquery.v2.ListFormatJob.job_reference:type_name -> google.cloud.bigquery.v2.JobReference
	26, // 12: google.cloud.bigquery.v2.ListFormatJob.error_result:type_name -> google.cloud.bigquery.v2.ErrorProto
	21, // 13: google.cloud.bigquery.v2.ListFormatJob.statistics:type_name -> google.cloud.bigquery.v2.JobStatistics
	19, // 14: google.cloud.bigquery.v2.ListFormatJob.configuration:type_name -> google.cloud.bigquery.v2.JobConfiguration
	22, // 15: google.cloud.bigquery.v2.ListFormatJob.status:type_name -> google.cloud.bigquery.v2.JobStatus
	11, // 16: google.cloud.bigquery.v2.JobList.jobs:type_name -> google.cloud.bigquery.v2.ListFormatJob
	25, // 17: google.cloud.bigquery.v2.GetQueryResultsRequest.start_index:type_name -> google.protobuf.UInt64Value
	27, // 18: google.cloud.bigquery.v2.GetQueryResultsRequest.max_results:type_name -> google.protobuf.UInt32Value
	27, // 19: google.cloud.bigquery.v2.GetQueryResultsRequest.timeout_ms:type_name -> google.protobuf.UInt32Value
	28, // 20: google.cloud.bigquery.v2.GetQueryResultsRequest.format_options:type_name -> google.cloud.bigquery.v2.DataFormatOptions
	29, // 21: google.cloud.bigquery.v2.GetQueryResultsResponse.schema:type_name -> google.cloud.bigquery.v2.TableSchema
	20, // 22: google.cloud.bigquery.v2.GetQueryResultsResponse.job_reference:type_name -> google.cloud.bigquery.v2.JobReference
	25, // 23: google.cloud.bigquery.v2.GetQueryResultsResponse.total_rows:type_name -> google.protobuf.UInt64Value
	30, // 24: google.cloud.bigquery.v2.GetQueryResultsResponse.rows:type_name -> google.protobuf.Struct
	31, // 25: google.cloud.bigquery.v2.GetQueryResultsResponse.total_bytes_processed:type_name -> google.protobuf.Int64Value
	32, // 26: google.cloud.bigquery.v2.GetQueryResultsResponse.job_complete:type_name -> google.protobuf.BoolValue
	26, // 27: google.cloud.bigquery.v2.GetQueryResultsResponse.errors:type_name -> google.cloud.bigquery.v2.ErrorProto
	32, // 28: google.cloud.bigquery.v2.GetQueryResultsResponse.cache_hit:type_name -> google.protobuf.BoolValue
	31, // 29: google.cloud.bigquery.v2.GetQueryResultsResponse.num_dml_affected_rows:type_name -> google.protobuf.Int64Value
	16, // 30: google.cloud.bigquery.v2.PostQueryRequest.query_request:type_name -> google.cloud.bigquery.v2.QueryRequest
	27, // 31: google.cloud.bigquery.v2.QueryRequest.max_results:type_name -> google.protobuf.UInt32Value
	33, // 32: google.cloud.bigquery.v2.QueryRequest.default_dataset:type_name -> google.cloud.bigquery.v2.DatasetReference
	27, // 33: google.cloud.bigquery.v2.QueryRequest.timeout_ms:type_name -> google.protobuf.UInt32Value
	34, // 34: google.cloud.bigquery.v2.QueryRequest.destination_encryption_configuration:type_name -> google.cloud.bigquery.v2.EncryptionConfiguration
	32, // 35: google.cloud.bigquery.v2.QueryRequest.use_query_cache:type_name -> google.protobuf.BoolValue
	32, // 36: google.cloud.bigquery.v2.QueryRequest.use_legacy_sql:type_name -> google.protobuf.BoolValue
	35, // 37: google.cloud.bigquery.v2.QueryRequest.query_parameters:type_name -> google.cloud.bigquery.v2.QueryParameter
	28, // 38: google.cloud.bigquery.v2.QueryRequest.format_options:type_name -> google.cloud.bigquery.v2.DataFormatOptions
	36, // 39: google.cloud.bigquery.v2.QueryRequest.connection_properties:type_name -> google.cloud.bigquery.v2.ConnectionProperty
	18, // 40: google.cloud.bigquery.v2.QueryRequest.labels:type_name -> google.cloud.bigquery.v2.QueryRequest.LabelsEntry
	31, // 41: google.cloud.bigquery.v2.QueryRequest.maximum_bytes_billed:type_name -> google.protobuf.Int64Value
	32, // 42: google.cloud.bigquery.v2.QueryRequest.create_session:type_name -> google.protobuf.BoolValue
	2,  // 43: google.cloud.bigquery.v2.QueryRequest.job_creation_mode:type_name -> google.cloud.bigquery.v2.QueryRequest.JobCreationMode
	3,  // 44: google.cloud.bigquery.v2.QueryRequest.query_results_format:type_name -> google.cloud.bigquery.v2.QueryRequest.QueryResultsFormat
	37, // 45: google.cloud.bigquery.v2.QueryRequest.arrow_serialization_options:type_name -> google.cloud.bigquery.v2.ArrowSerializationOptions
	29, // 46: google.cloud.bigquery.v2.QueryResponse.schema:type_name -> google.cloud.bigquery.v2.TableSchema
	20, // 47: google.cloud.bigquery.v2.QueryResponse.job_reference:type_name -> google.cloud.bigquery.v2.JobReference
	23, // 48: google.cloud.bigquery.v2.QueryResponse.job_creation_reason:type_name -> google.cloud.bigquery.v2.JobCreationReason
	25, // 49: google.cloud.bigquery.v2.QueryResponse.total_rows:type_name -> google.protobuf.UInt64Value
	30, // 50: google.cloud.bigquery.v2.QueryResponse.rows:type_name -> google.protobuf.Struct
	31, // 51: google.cloud.bigquery.v2.QueryResponse.total_bytes_processed:type_name -> google.protobuf.Int64Value
	32, // 52: google.cloud.bigquery.v2.QueryResponse.job_complete:type_name -> google.protobuf.BoolValue
	26, // 53: google.cloud.bigquery.v2.QueryResponse.errors:type_name -> google.cloud.bigquery.v2.ErrorProto
	32, // 54: google.cloud.bigquery.v2.QueryResponse.cache_hit:type_name -> google.protobuf.BoolValue
	31, // 55: google.cloud.bigquery.v2.QueryResponse.num_dml_affected_rows:type_name -> google.protobuf.Int64Value
	38, // 56: google.cloud.bigquery.v2.QueryResponse.session_info:type_name -> google.cloud.bigquery.v2.SessionInfo
	39, // 57: google.cloud.bigquery.v2.QueryResponse.dml_stats:type_name -> google.cloud.bigquery.v2.DmlStats
	40, // 58: google.cloud.bigquery.v2.QueryResponse.arrow_schema:type_name -> google.cloud.bigquery.v2.ArrowSchema
	41, // 59: google.cloud.bigquery.v2.QueryResponse.arrow_record_batch:type_name -> google.cloud.bigquery.v2.ArrowRecordBatch
	5,  // 60: google.cloud.bigquery.v2.JobService.CancelJob:input_type -> google.cloud.bigquery.v2.CancelJobRequest
	7,  // 61: google.cloud.bigquery.v2.JobService.GetJob:input_type -> google.cloud.bigquery.v2.GetJobRequest
	8,  // 62: google.cloud.bigquery.v2.JobService.InsertJob:input_type -> google.cloud.bigquery.v2.InsertJobRequest
	9,  // 63: google.cloud.bigquery.v2.JobService.DeleteJob:input_type -> google.cloud.bigquery.v2.DeleteJobRequest
	10, // 64: google.cloud.bigquery.v2.JobService.ListJobs:input_type -> google.cloud.bigquery.v2.ListJobsRequest
	13, // 65: google.cloud.bigquery.v2.JobService.GetQueryResults:input_type -> google.cloud.bigquery.v2.GetQueryResultsRequest
	15, // 66: google.cloud.bigquery.v2.JobService.Query:input_type -> google.cloud.bigquery.v2.PostQueryRequest
	6,  // 67: google.cloud.bigquery.v2.JobService.CancelJob:output_type -> google.cloud.bigquery.v2.JobCancelResponse
	4,  // 68: google.cloud.bigquery.v2.JobService.GetJob:output_type -> google.cloud.bigquery.v2.Job
	4,  // 69: google.cloud.bigquery.v2.JobService.InsertJob:output_type -> google.cloud.bigquery.v2.Job
	42, // 70: google.cloud.bigquery.v2.JobService.DeleteJob:output_type -> google.protobuf.Empty
	12, // 71: google.cloud.bigquery.v2.JobService.ListJobs:output_type -> google.cloud.bigquery.v2.JobList
	14, // 72: google.cloud.bigquery.v2.JobService.GetQueryResults:output_type -> google.cloud.bigquery.v2.GetQueryResultsResponse
	17, // 73: google.cloud.bigquery.v2.JobService.Query:output_type -> google.cloud.bigquery.v2.QueryResponse
	67, // [67:74] is the sub-list for method output_type
	60, // [60:67] is the sub-list for method input_type
	60, // [60:60] is the sub-list for extension type_name
	60, // [60:60] is the sub-list for extension extendee
	0,  // [0:60] is the sub-list for field type_name
}

func init() { file_google_cloud_bigquery_v2_job_proto_init() }
func file_google_cloud_bigquery_v2_job_proto_init() {
	if File_google_cloud_bigquery_v2_job_proto != nil {
		return
	}
	file_google_cloud_bigquery_v2_arrow_proto_init()
	file_google_cloud_bigquery_v2_data_format_options_proto_init()
	file_google_cloud_bigquery_v2_dataset_reference_proto_init()
	file_google_cloud_bigquery_v2_encryption_config_proto_init()
	file_google_cloud_bigquery_v2_error_proto_init()
	file_google_cloud_bigquery_v2_job_config_proto_init()
	file_google_cloud_bigquery_v2_job_creation_reason_proto_init()
	file_google_cloud_bigquery_v2_job_reference_proto_init()
	file_google_cloud_bigquery_v2_job_stats_proto_init()
	file_google_cloud_bigquery_v2_job_status_proto_init()
	file_google_cloud_bigquery_v2_query_parameter_proto_init()
	file_google_cloud_bigquery_v2_session_info_proto_init()
	file_google_cloud_bigquery_v2_table_schema_proto_init()
	file_google_cloud_bigquery_v2_job_proto_msgTypes[12].OneofWrappers = []any{
		(*queryRequest_ArrowSerializationOptions)(nil),
	}
	file_google_cloud_bigquery_v2_job_proto_msgTypes[13].OneofWrappers = []any{
		(*queryResponse_ArrowSchema)(nil),
		(*queryResponse_ArrowRecordBatch)(nil),
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_google_cloud_bigquery_v2_job_proto_rawDesc), len(file_google_cloud_bigquery_v2_job_proto_rawDesc)),
			NumEnums:      4,
			NumMessages:   15,
			NumExtensions: 0,
			NumServices:   1,
		},
		GoTypes:           file_google_cloud_bigquery_v2_job_proto_goTypes,
		DependencyIndexes: file_google_cloud_bigquery_v2_job_proto_depIdxs,
		EnumInfos:         file_google_cloud_bigquery_v2_job_proto_enumTypes,
		MessageInfos:      file_google_cloud_bigquery_v2_job_proto_msgTypes,
	}.Build()
	File_google_cloud_bigquery_v2_job_proto = out.File
	file_google_cloud_bigquery_v2_job_proto_goTypes = nil
	file_google_cloud_bigquery_v2_job_proto_depIdxs = nil
}
